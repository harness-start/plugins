...
Prompt、Skill 段落、Hook、工具、agent 和验证阶段都代表一个关于模型能力或风险的假设。新增或扩大这些组件前，先固定模型/provider/版本、effort、宿主、任务、预算、干净消费者安装和 outcome gate，然后一次只移除一个组件组，比较同一任务的结果。

正确性、完整性和任务专属质量门槛是主要指标；token、调用次数、时延和成本只用于判断收益是否值得。若移除后质量相同，选择更简单的变体；若组件没有可重复的结果增益，不因 Hook 激活、Skill 加载或多一次模型调用而保留它。模型代际、宿主行为或主要路由变化后，应重新消融可能已经过时的脚手架。

日常实现中的消融属于 `session-governance` 的开放式工程方法：锁定可观察合同后，只对本次任务引入或修改的结构逐项删减，并在最后一次删减或恢复后重跑同一验证。它不是可机械判断的 Stop 条件，不创建全局账本，也不授权清理无关技术债。
