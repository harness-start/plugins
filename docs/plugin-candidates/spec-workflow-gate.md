...
`harness-starter` 的 `spec-workflow-artifact-gate` 与 `ai-experts` 的 `spec-plan-artifact-gate` 都会在写下游产物前检查 sibling `spec.md`。本仓的 `intent-discovery` 只在首轮前置探索上下文，`reasoning-methods` 只提供思考方法；它们都不验证磁盘上的规格产物依赖。
