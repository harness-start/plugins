...
| `tdd-sequence-gate` | 由 `tdd-guard` 收窄替代 | 不再宣称从命令时序证明 RED/GREEN；新插件只对测试与实现文件的真实字节顺序、六语言实体身份和完整目录镜像做写前硬门禁，不使用全局 basename/simple name 解锁。缺陷修复的完整 BEFORE/AFTER 行为证据仍由 `behavioral-regression-guard` 提供。 |
