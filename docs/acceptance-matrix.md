...
| `engineering-workflow` delegation | Offline protocol only | 1.1.0 | Explicit `delegate <codex\|claude\|agy\|grok\|pi>` routing, dirty-baseline audit, index boundary | ⏳ All five bundled relay help protocols and the owner route are covered offline; a fake Codex process proves dirty-baseline deltas and index-change failure. No real provider session has been accepted, so every provider remains live-unverified. | Focused unit/protocol tests; no model or provider network call | Relay artifacts in a caller-selected or temporary directory; no project state database | Live acceptance intentionally not claimed |
