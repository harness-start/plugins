...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `tdd-guard` | Claude + Codex | 0.1.0 | `01-block-source-first`、`02-allow-test-first` | ✅ 两宿主均在实现文件产生副作用前拒绝 source-first 写入；test-first 场景先记录当前测试文件 SHA 和关联符号，再允许独立的实现写入。Codex 的 Post Hook 静默记录，避免 DeepSeek 工具输出丢失 | 2026-08-13 Docker 定向汇总 `4/0/0`；honesty gate `202/0`；13 项离线测试覆盖六语言路径、注释伪声明、真实 Codex `tool_input.command`、关联关系、混合补丁、失败关闭与状态隐私 | 平台插件数据目录中的 session/workspace 哈希状态；0600 原子写；不保存测试源码 | — |
