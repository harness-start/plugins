...
| `professional-writing` | Claude + Codex | 1.2.0 | `07-actionable-response`, `08-visual-explanation` | Pass: ordinary action-heavy requests load the actionable method and preserve task order, commands, and failure handoff without diagnosis or invented time; visual explanations preserve the four-node data flow in one bounded inline view without creating HTML or workspace files | 2026-08-21 Docker targeted `4/0/0`; honesty `274/0` | No persistent state | — |
