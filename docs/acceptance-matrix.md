...
| `subagent-workflow-guard` | Claude + Codex | 0.2.0 | `01-application-dispatch` | ⚠️ Claude 的 `Agent` 已证明在 `SubagentStart` 前硬拒绝无申请单派发；Codex 0.147 的 `collaboration.spawn_agent` 不产生 dispatch `PreToolUse`，因此不宣称 Codex 派发硬门禁；`SubagentStart` 仅在子会话身份与 task/application ID 精确一致时绑定 | 2026-08-14：Node 20 完整 CI 通过；Codex 0.147 Docker live acceptance 通过并观察真实子会话；Claude 分支在模型会话前被既有 community skill 安装布局不兼容阻断 | 平台各自 plugin data 下的 session/application 状态；交互 CLI mailbox 位于 Git 私有目录中的 host/session 哈希路径；0600 文件 | — |
