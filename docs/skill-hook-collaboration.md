...
同一插件、同一生命周期事件必须优先使用一个 dispatcher，在进程内按明确顺序执行检查。禁止为每条细规则注册一个 command Hook。
...
| G 界面工艺 | 机械检测 UI 反模式，不替代领域语法门禁 | 路由 + Post 扫描；Stop 不得冒充已回滚 | floor / critique 方法 |
...
Skill 提供工艺底线与审查方法。检测本身不得依赖 Skill 是否加载。
