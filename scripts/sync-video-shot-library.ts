...
async function main() {
  ...
  const output = resolve(argument("--output") ?? join(projectRoot, "plugins/artifact-production/modules/video/src/generated/shot-library-data.ts"));
  ...
  const licenseDirectory = join(projectRoot, "plugins/artifact-production/modules/video/licenses/video-shotcraft");
