...
export async function runSync() {
  ...
  const output = resolve(argument("--output") ?? join(projectRoot, "plugins/artifact-production/src/domains/video/generated/shot-library-data.ts"));
  ...
  const licenseDirectory = join(projectRoot, "plugins/artifact-production/licenses/video-shotcraft");
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) await runSync();
