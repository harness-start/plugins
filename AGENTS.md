...
- Design and implement every published plugin for a clean consumer environment: never select its architecture, omit required behavior, or claim coverage based on Skills, hooks, tools, caches, or session state visible only in the current development session or workspace. Bundle required mechanisms or declare installable dependencies; assume every undeclared capability is absent after publication, and verify acceptance in that isolated model. Community Skill dependencies must declare their name and current upstream source without revision locks. Treat `vendor-skills/` and its deterministic index as the only consumer installation source for declared community Skills: refresh it only through `scripts/update-vendor-skills.sh`, commit both content and index, permit no install-time external-source fallback, and fail closed on missing files or digest drift.
