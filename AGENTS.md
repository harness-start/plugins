...
- Owner runtime code lives in `plugins/<owner>/src/`; private module runtime code lives in `plugins/<owner>/modules/<module>/src/`. Each build unit is bundled to its own committed, self-contained `dist/`, and every generated `.mjs` records a SHA-256 over that unit's `src/**/*.ts` plus shared `core/src/**/*.ts`. Run `npm run build` after source changes and include every refreshed `dist/` in each push; project hooks run `npm run ensure:dist` before project `git push`/`git send-pack`, rebuild mismatches, and stop that push until refreshed artifacts are committed. `npm run check:dist` must pass without modifying files.
- Publish exactly the eight AIO owners listed in both marketplace catalogs. Treat `plugins/<owner>/modules/*` as private, self-contained implementation modules, never as marketplace entries or cross-owner dependencies. Installation is fixed and all-on: do not add capability profiles or FDE/OPC runtime branches. Owners with deterministic tools expose only `dist/cli/harness.mjs <resource> <action>` as their public CLI protocol.
