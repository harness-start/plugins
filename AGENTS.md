...
- Treat every Prompt, Skill section, Hook, tool, agent, and verification stage as a removable hypothesis. For a claimed improvement, hold the model, host, task, budget, and outcome gate fixed; remove one component group at a time; prefer the simpler equal-quality variant; and re-run the ablation when the model generation or host behavior changes. Do not use token count, tool calls, or deletion volume as substitutes for task outcomes.
...
- For substantial changes, follow contract → challenge or baseline → minimal change → targeted verification → complete applicable verification → adversarial review → evidence report. Behavior-changing code requires an edited public-seam test and observed RED before production edits; refactors require a GREEN baseline.
- For a narrow, low-risk change with a stable interface and direct oracle, stop after fresh focused evidence; do not infer a full repository suite from completion, commit, or pull-request intent. Explicit user, project, and CI gates remain authoritative.
- Completion command evidence must follow the last mutation and run in the current user-prompt epoch at the selected verification scope. Missing workflow evidence permits only `blocked` or `needs_context`, not `done_with_concerns`.
- Owner runtime code lives in `plugins/<owner>/src/` and every owner has one committed, self-contained `dist/`. Internal concerns use source namespaces such as `src/domains/<domain>/`, but they share the owner Hook, CLI, test, acceptance, Skill, license, and build boundaries. `plugins/<owner>/modules/*` directories are forbidden. Every generated `.mjs` records a SHA-256 over the owner's `src/**/*.ts` plus shared `core/src/**/*.ts`. Run `npm run build` after source changes and include every refreshed `dist/` in each push; project hooks run `npm run ensure:dist` before project `git push`/`git send-pack`, rebuild mismatches, and stop that push until refreshed artifacts are committed. `npm run check:dist` must pass without modifying files.
