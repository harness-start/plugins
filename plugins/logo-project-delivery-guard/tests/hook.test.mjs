...
const ENTRY = fileURLToPath(new URL("../scripts/logo-project-delivery-guard.mjs", import.meta.url));
...
test("pre hook rejects a node eval command containing an approved wrapper substring", async () => {
  ...
  try {
    const wrapper = fileURLToPath(new URL("../scripts/tools/project-release.mjs", import.meta.url));
...
test("pre hook allows only an exact registered render invocation for the project root", async () => {
  ...
  try {
    mkdirSync(join(root, "artifacts", "logo", "orbit"), { recursive: true });
    const wrapper = fileURLToPath(new URL("../scripts/tools/project-render.mjs", import.meta.url));
