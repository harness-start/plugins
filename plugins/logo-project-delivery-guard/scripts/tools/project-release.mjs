...
import { createLogoReceipt, validateLogoModel, validateLogoReceipt } from "../lib/contract.mjs";
...
async function collect(directory, files, digests, count) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await collect(absolute, files, digests, count);
      ...
      if (++count.value > 2048) throw new Error("PROJECT_FILE_LIMIT_EXCEEDED");
      const bytes = await readFile(absolute);
      if (bytes.byteLength > 64 * 1024 * 1024) throw new Error(`PROJECT_FILE_SIZE_EXCEEDED:${entry.name}`);
...
async function load() {
  ...
  return { artifactId: basename(root), files, digests, plan: parse("plan.contract.json"), project: parse("logo.project.json") };
}
...
async function main() {
  ...
  try {
    ...
    if (!validateLogoReceipt(await load())) throw new Error("written receipt did not verify against current files");
    complete = true;
