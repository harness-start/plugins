...
var HIGH_RISK_PROMPT = /\b(?:auth(?:entication|orization)?|security|schema|migrat\w*|database|persistence|concurren\w*|race condition|data integrity|deploy\w*|release|runtime state|recovery|rollback|observability|cross-module|multi-module)\b|认证|授权|安全|跨模块|数据库|持久化|迁移|并发|数据完整性|部署|发布|运行态|恢复|回滚|可观测/iu;
var PUBLIC_API_CHANGE_PROMPT = /\b(?:add|introduce|extend|redesign|alter|change|modify|update|migrate|remove|rename|replace)\s+(?:(?:a|the)\s+)?public\s+api\b|(?:新增|引入|扩展|重构|修改|变更|迁移|删除|重命名|替换)\s*公共\s*api/iu;
var FOCUSED_SCOPE_PROMPT = /\b(?:narrow|small|low-risk|direct (?:test )?oracle|stable public api)\b|小改动|小问题|局部|低风险|稳定.{0,12}(?:接口|api)|直接.{0,12}(?:测试|验证)/iu;
