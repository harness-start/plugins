...
import { DEFAULT_CONFIG, resolveConfig } from "../../../src/domains/discipline/lib/config.js";
import {
  countRemotePolls,
  ...
  normalizeCommand,
} from "../../../src/domains/discipline/lib/execution-loop-policy.js";

const ENTRY = fileURLToPath(new URL("../../../dist/hooks/dispatcher.mjs", import.meta.url));

function workspace(prefix = "execution-discipline-") {
...
function runEntry(mode, event, env = {}) {
  return new Promise((resolvePromise, reject) => {
    const host = mode === "failure" ? "claude" : "codex";
    const child = spawn(process.execPath, [ENTRY, host, ({ "session-start": "SessionStart", pre: "PreToolUse", post: "PostToolUse", failure: "PostToolUseFailure", stop: "Stop", session: "SessionStart", prompt: "UserPromptSubmit", "user-prompt": "UserPromptSubmit", subagent: "SubagentStart", "subagent-stop": "SubagentStop" } as Record<string, string>)[mode] ?? mode], {
      env: { ...process.env, ...env },
...
test("edit loop reports, blocks, then starts a fresh cycle", async (context) => {
  ...
  writeFileSync(join(root, ".execution-discipline.mjs"), [
    "export default {",
...
test("successful verification clears edit counters for the session and workspace", async (context) => {
  ...
  const data = mkdtempSync(join(tmpdir(), "execution-loop-state-"));
  context.after(() => { rmSync(root, { recursive: true, force: true }); rmSync(data, { recursive: true, force: true }); });
  writeFileSync(join(root, ".execution-discipline.mjs"), "export default { editLoop: { reportAt: 2, blockAt: 3 } };\n");
  const env = { PLUGIN_DATA: data };
...
test("remote polling remains report-only by default", async (context) => {
  ...
  const data = mkdtempSync(join(tmpdir(), "execution-loop-state-"));
  context.after(() => { rmSync(root, { recursive: true, force: true }); rmSync(data, { recursive: true, force: true }); });
  writeFileSync(join(root, ".execution-discipline.mjs"), "export default { polling: { queryBudgetCount: 1 } };\n");
...
test("only the Codex owner route advertises Codex function-level wait observation", () => {
  const toolsFor = (host: "claude" | "codex") => new Set(
    readModuleRoutes(import.meta.url, host, "discipline").PreToolUse
      .flatMap((route) => (route.matcher ?? "").split("|")),
  );
  const codexTools = toolsFor("codex");
  const claudeTools = toolsFor("claude");
...
test("state persistence uses state and creates only the guard-local gitignore", async (context) => {
  ...
  assert.equal(existsSync(join(root, ".execution-discipline", "state")), true);
  assert.equal(existsSync(join(root, ".execution-discipline", ".state")), false);
  assert.equal(readFileSync(join(root, ".execution-discipline", ".gitignore"), "utf8"), "*\n");
...
test("state does not persist raw paths, commands, or command output", async (context) => {
  ...
  const directory = join(root, ".execution-discipline", "state");
  ...
  assert.equal(existsSync(join(data, "execution-discipline")), false);
});
