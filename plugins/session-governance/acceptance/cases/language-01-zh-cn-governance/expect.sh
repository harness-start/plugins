...
if [ "${ACCEPT_HOST:-}" = "codex" ] && grep -Fq 'No tool output found for tool call' "${ACCEPT_LOG}"; then
  echo "expect fail: Codex provider lost a tool result after PostToolUse" >&2
  exit 1
fi
require_guard_hook_signal "${MARKERS_LANGUAGE_OUTPUT_FEEDBACK}|${MARKERS_LANGUAGE_OUTPUT_GATE}|${MARKERS_STOP_BLOCK}"
