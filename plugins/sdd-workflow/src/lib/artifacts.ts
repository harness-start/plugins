...
function hasSymlink(path, stop) {
  ...
  while (isWithin(boundary, current)) {
    try { if (lstatSync(current).isSymbolicLink()) return true; } catch { /* absent leaf is allowed */ }
...
export function inspectChange(changeDir) {
  ...
  const plan = values["plan.md"] === undefined ? null : validatePlanText(values["plan.md"], spec);
  const tasks = values["tasks.md"] === undefined ? null : validateTasksText(values["tasks.md"], spec, plan, dirname(dirname(absolute)));
