...
test("Codex parent-lane oracle accepts parent writes outside worker lanes", () => {
  assert.equal(parentAvoidedLanes([
    ...
    call("exec_command", "exec-absolute-workspace", {
      ...
      workdir: "/out/sdd-workflow__04-multi-task-context-isolation__codex/workspace",
    }),
  ]), true);
  assert.equal(parentAvoidedLanes([
    {
  ...
  assert.equal(parentAvoidedLanes([
    call("exec_command", "exec-skill", {
      cmd: "cat /out/example/codex-home/plugins/cache/harness-start/sdd-workflow/0.2.0/skills/sdd-build/SKILL.md",
    }),
...
test("Codex parent-lane oracle rejects apply_patch and shell access to worker lanes", () => {
  ...
  assert.equal(parentAvoidedLanes([
    call("exec_command", "exec-substitution", {
      cmd: "cat /out/x$(cat${IFS}lanes/orchard/request.md)/sdd-workflow/0.2.0/skills/sdd-build/SKILL.md",
    }),
  ]), false);
  assert.equal(parentAvoidedLanes([
    call("exec_command", "exec-backtick", {
      cmd: "cat /out/x`cat lanes/orchard/request.md`/sdd-workflow/0.2.0/skills/sdd-build/SKILL.md",
    }),
  ]), false);
});
