/**
 * JVM dependency lockfile guard (PreToolUse).
 * Failure mode: fail-closed (deny) with a blockingContract.
 */

import { basename } from "node:path";

export const LOCKFILE_NAMES = ["gradle.lockfile"].map((n) => n.toLowerCase());
...
export function lockfileDenyMessage(targets) {
  const generatedBy = "Gradle dependency locking";
  return [
    `[JVM Dependency Lockfile Guard] 已拦截依赖 lock 文件修改`,
    "",
    `目标：${targets.join(", ")}`,
    "",
