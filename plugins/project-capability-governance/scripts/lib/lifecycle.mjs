...
import { forgetNotice, parseProposal } from "./proposals.mjs";

const PROPOSAL_ID = /^[a-z0-9][a-z0-9-]{2,63}$/u;
const ACTIVE_STATUSES = ["pending", "reviewing", "deferred"];
...
export async function findProposal(projectRoot, proposalId) {
...
export async function deleteProposal(projectRoot, proposalId, outcome) {
...
export async function startProposal(projectRoot, proposalId) {
  const found = await findProposal(projectRoot, proposalId);
...
export async function blockProposal(projectRoot, proposalId, reason) {
  const found = await findProposal(projectRoot, proposalId);
...
export async function deferProposal(projectRoot, proposalId, condition) {
  const found = await findProposal(projectRoot, proposalId);
...
export async function reopenProposal(projectRoot, proposalId) {
  const found = await findProposal(projectRoot, proposalId);
