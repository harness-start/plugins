...
export function parseProposal(content, fileName = "") {
...
export function validateProposalDocument(content, fileName) {
  ...
  const bulletCount = (heading) => section(heading)
    .split(/\r?\n/u)
  ...
  if (parsed.kind === "hook") {
    if (!new Set(["ordinary", "severe"]).has(fields.risk)) {
...
export function proposalLocation(projectRoot, target) {
...
export function isProposalInboxTarget(projectRoot, target) {
...
export async function ensureCapabilityWorkspace(projectRoot) {
...
async function readNoticeState(projectRoot) {
  try {
    ...
    return parsed && typeof parsed.notified === "object" && !Array.isArray(parsed.notified)
      ? parsed
      : { version: 1, notified: {} };
  } catch {
...
export async function consumeNoticeDelta(projectRoot) {
  const root = resolve(projectRoot);
...
export async function forgetNotice(projectRoot, proposalId) {
