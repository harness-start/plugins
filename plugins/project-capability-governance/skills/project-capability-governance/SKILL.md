...
Before mutation, review the causal chain, reuse evidence, and whether a Hook is justified. For a difficult or high-impact judgment, you may ask an ordinary read-only subagent in plain language to challenge that reasoning. Treat its response as advice: the parent agent must inspect the cited evidence, make the decision, and remain responsible for every change. Then show the human the exact paths, whether each path is new or merged, the verification command, and the rollback boundary. Preserve unrelated changes.
