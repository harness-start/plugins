...
import { isOfficialWriterCommand, loadLedger, scanLedgers } from "../src/lib/ledger.js";
import { activateBug, affectBugs, closeLedger, initLedger, pauseLedger, resumeLedger, statusLedger } from "../src/lib/writer.js";
import {
  bindAfterWriter,
...
import { extractWorkOrder, validateWorkOrder } from "../src/lib/work-order.js";

const HOOK_SRC = fileURLToPath(new URL("../src/entries/hooks/debugging-workflow-guard.ts", import.meta.url));
const TSX = fileURLToPath(new URL("../../../node_modules/tsx/dist/cli.mjs", import.meta.url));
const CLI_SRC = fileURLToPath(new URL("../src/entries/cli/debug-workflow.ts", import.meta.url));
...
function runCli(args, cwd) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(process.execPath, [TSX, CLI_SRC, ...args, "--cwd", cwd], {
      env: process.env,
      stdio: ["pipe", "pipe", "pipe"],
    });
    let stdout = "";
...
test("writer resume with the same id and a higher epoch rebinds the session", () => {
  ...
  const first = bindAfterWriter({
    ...
    command: `node ${CLI_SRC} init --cwd ${root} --slug resume-rebind`,
    stdout: JSON.stringify(created),
  });
  ...
  bindAfterWriter({
    cwd: root,
    sessionId: "resume-session",
    command: `node ${CLI_SRC} close --cwd ${root} --id ${created.id}`,
  ...
  const rebound = bindAfterWriter({
    cwd: root,
    sessionId: "resume-session",
    command: `node ${CLI_SRC} resume --cwd ${root} --id ${created.id} --bug BUG-001`,
...
test("official writer command is recognized and a claim event is visible", () => {
  assert.equal(isOfficialWriterCommand("node /plugins/debugging-workflow-guard/dist/cli/debug-workflow.mjs init --slug login"), true);
