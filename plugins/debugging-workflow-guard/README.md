...
4. 生产修改前必须有独立 `DBG_REVIEW_REQUEST diagnosis` 审批。Codex 的 `SubagentStart`/`SubagentStop` 会从受校验 transcript 取父线程 ID，把 child-session 事件归回父会话并自动注入 nonce 与 Work Order evidence；不需要手工调用 hook 脚本。缺陷标为 resolved 前，必须有已应用的修复修改、修改前失败、受支持的假设与根因、最后一次相关修改后原复现成功、回归成功以及非失败的清理回执；`Stop` 还会独立扫描 Work Order debug marker。`architecture-review` 需要另一个 agent 的 `DBG_REVIEW_REQUEST architecture` 审批。
