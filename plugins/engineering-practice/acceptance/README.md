...
- `07-mixed-boundary`：UserPromptSubmit 对联合 normalization 做窄路由；验证 agent 不把当前异常误当兼容合同，在首个有损变换前处理一空一非空边界，并仍复用共同输出装配。
- `08-stable-order-primitive`：UserPromptSubmit 对依赖顺序做窄路由；验证 agent 发现并复用仓库已有 primitive，覆盖独立链 stable ready-frontier、重复项、cycle fallback 和精确诊断。
