...
- `07-mixed-boundary`：联合 normalization 会抹掉一空一非空分量；验证 agent 在首个有损变换前处理边界，并仍复用共同输出装配。
- `08-stable-order-primitive`：多条依赖链需要稳定合并；验证 agent 发现并复用仓库已有 primitive，覆盖独立链 tie-break、重复项、cycle fallback 和精确诊断。
