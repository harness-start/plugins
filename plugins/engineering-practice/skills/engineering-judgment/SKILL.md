...
For ordering or dependency behavior, search the repository and language standard library for an existing stable primitive before implementing a graph or merge algorithm. If no suitable primitive exists, define the unresolved degrees of freedom explicitly. Before completion, add a direct durable test with two completely disjoint chains containing at least two items each to prove whether ready heads precede either tail; single-item chains and a three-chain example with a cross-chain dependency are not equivalent. Also cover shared and duplicate items, a genuine cycle and its fallback, and the exact warning or error contract. A single requested ordering cannot prove those semantics, and flattened first-appearance order is not automatically a valid stable partial-order rule.
