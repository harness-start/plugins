...
const STAGE_RANK: Record<PosterStage, number> = {
  source: 0,
  design: 1,
  render: 2,
  probe: 3,
...
const VARIANT_DIRECTORY =
  /^(?<index>[0-9]{3})-(?<slug>[a-z0-9]+(?:-[a-z0-9]+)*)$/u;
const LAYER_SOURCE =
  /^(?<index>[0-9]{3})-(?<role>background|media|overlay|decoration|title|body|metadata|brand|cta)-(?<slug>[a-z0-9]+(?:-[a-z0-9]+)*)\.tsx$/u;
const OWNER_VIOLATION =
  /(?:\bzIndex\s*:|from\s+["'](?:satori|@resvg\/resvg-js|node:|fs|child_process|http|https|net|tls|dns)|\b(?:import|require|fetch|setTimeout|setInterval)\s*\(|\b(?:process|globalThis|performance|crypto)\.|\b(?:Date\.now|Math\.random)\s*\(|\bnew\s+Date\b|\b(?:useState|useEffect|useLayoutEffect)\s*\(|<\s*(?:script|style|link|iframe)\b|https?:\/\/)/u;
const GENERATED_PATH =
  /^(?:dist\/|evidence(?:\.|\/)|review\.poster\.json$|release\.manifest\.json$|receipt\.release\.json$|\.poster-delivery-journal\.json$|\.tmp\/poster-guard\/)/u;
const SUBJECT_EXCLUDED_PATH =
  /^(?:dist\/|evidence(?:\.|\/)|review\.poster\.json$|release\.manifest\.json$|receipt\.release\.json$|\.poster-delivery-journal\.json$|\.tmp\/)/u;
...
const textOf = (value: unknown): string =>
  Buffer.isBuffer(value)
    ...
    : typeof value === "string"
      ...
      : "";
const rawBytes = (
  model: PosterModel | null | undefined,
  path: string,
): Buffer =>
  model?.bytes?.[path] ??
  (Buffer.isBuffer(model?.files?.[path])
    ? (model.files[path] as Buffer)
    : Buffer.from(textOf(model?.files?.[path])));
const digestOf = (
  model: PosterModel | null | undefined,
  path: string,
): string => model?.digests?.[path] ?? sha256(rawBytes(model, path));
const stageAtLeast = (stage: PosterStage, expected: PosterStage) =>
...
function parseJson(
  ...
  try {
    return JSON.parse(textOf(files[path])) as unknown;
  } catch {
    findings.push(
...
export function inspectPosterSvg(bytes: Buffer): {
  width: number;
  height: number;
  ...
  if (
    bytes.byteLength < 40 ||
    /<!DOCTYPE|<!ENTITY|<(?:script|foreignObject)\b|\son[a-z]+\s*=/iu.test(
      source,
    )
  )
    throw new Error("SVG_UNSAFE");
  if (
    /\b(?:href|xlink:href)\s*=\s*["'](?:https?:|file:|\/\/)|\burl\(\s*["']?(?:https?:|file:|\/\/)/iu.test(
      source,
    )
  )
  ...
  return {
    width,
    height,
    viewBox: viewBoxParts as [number, number, number, number],
  };
}

function decodePosterPng(bytes: Buffer): {
  width: number;
  height: number;
  ...
  try {
    ...
    while (cursor < bytes.byteLength) {
      ...
      if (
        pngCrc32(Buffer.concat([type, bytes.subarray(dataStart, dataEnd)])) !==
        expectedCrc
      )
        throw new Error("PNG_INVALID");
...
export function inspectPosterPng(bytes: Buffer): {
  width: number;
  height: number;
  alphaCoverage: number;
} {
...
export function posterForegroundMask(
  bytes: Buffer,
  canvasHex: string,
  threshold = 18,
): {
  ...
  const canvas = [0, 2, 4].map((offset) =>
    Number.parseInt(canvasHex.slice(offset, offset + 2), 16),
  );
  ...
  for (let index = 0; index < mask.length; index += 1) {
    ...
    const distance =
      [0, 1, 2].reduce((sum, channel) => {
        const composite =
          (rgba[offset + channel] ?? 0) * alpha +
          (canvas[channel] ?? 0) * (1 - alpha);
...
export function computePosterSubjectDigest(
  model: PosterModel | null | undefined,
): string {
  const paths = Object.keys(model?.files ?? {})
    .filter(
...
function variantRecords(model: PosterModel | null | undefined): JsonRecord[] {
  try {
    return list(
      record(JSON.parse(textOf(model?.files?.["src/variants/manifest.json"])))
        .variants,
    ).map(record);
  } catch {
    return [];
...
function receiptOutputPaths(model: PosterModel | null | undefined): string[] {
  return [
    ...
    ...Object.keys(model?.files ?? {})
      .filter((path) => /^evidence\/layers\/.+\.(?:png|svg)$/u.test(path))
      .sort(),
...
export function createPosterReleaseManifest(
  model: PosterModel | null | undefined,
) {
  return {
...
export function createPosterReceipt(
  model: PosterModel | null | undefined,
): PosterReceipt {
  return {
    schemaVersion: 2,
...
export function validatePosterReceipt(
  model: PosterModel | null | undefined,
): boolean {
  try {
    const actual = record(
...
function validateBase(
  ...
  if (
    ...
    !STAGE_SET.has(String(plan.targetStage)) ||
  ...
  const artInvalid =
    ...
    [
      ...
      (key) =>
        typeof letterform[key] !== "string" || !String(letterform[key]).trim(),
    ) ||
  ...
  if (
    skills.schema !== SKILL_COMPOSITION_SCHEMA ||
    workers.length !== REQUIRED_ADVISORS.size ||
    workers.some((worker) => Object.hasOwn(worker, "revision")) ||
    [...REQUIRED_ADVISORS].some(
      (name) =>
        !["used", "skipped", "unavailable"].includes(
  ...
  if (
    assets.schema !== ASSET_MANIFEST_SCHEMA ||
    entries.some(
      (asset) =>
        ...
        !["project", "user", "generated", "licensed", "public-domain"].includes(
          String(asset.sourceType),
        ) ||
        !(String(asset.path) in files) ||
        digestOf(model, String(asset.path)) !== asset.sha256 ||
        (asset.sourceType === "generated" &&
          (!asset.tool ||
            !asset.model ||
            !/^[a-f0-9]{64}$/u.test(String(asset.promptDigest)))) ||
        (["licensed", "public-domain"].includes(String(asset.sourceType)) &&
          (!/^https?:\/\//u.test(String(asset.sourceUrl)) ||
            typeof asset.rightsStatus !== "string" ||
            !asset.rightsStatus.trim())),
    )
  )
    findings.push(
      finding(
  ...
  const registryValid =
    ...
    fontRegistry.every((font) => {
      const fontFiles = list(font.files).map(record);
      return (
        typeof font.family === "string" &&
        font.family.trim().length > 0 &&
        typeof font.package === "string" &&
        /^@[a-z0-9-]+\/[a-z0-9-]+$/u.test(font.package) &&
        fontFiles.length > 0 &&
        fontFiles.every(
          (file) =>
            typeof file.path === "string" &&
            /^[a-z0-9][a-z0-9._-]*\.woff2?$/iu.test(file.path) &&
            Number.isInteger(file.weight) &&
            Number(file.weight) >= 100 &&
            Number(file.weight) <= 900 &&
            ["latin", "cjk"].includes(String(file.script)),
        )
      );
    });
  ...
  if (
    ...
    Object.values(typography)
      ...
      .some(
        (role) =>
          ...
          !Number.isFinite(role.lineHeightPx) ||
          Number(role.lineHeightPx) < Number(role.sizePx) ||
          !Number.isFinite(role.letterSpacingEm) ||
          Number(role.letterSpacingEm) < -0.08 ||
          Number(role.letterSpacingEm) > 0.5 ||
    ...
    Number(spacing.safeAreaPx) <= 0 ||
    Number(spacing.baseUnitPx) <= 0 ||
    Number(spacing.paragraphGapPx) < 0 ||
    contrastPairs.length === 0 ||
    contrastPairs.some(
      (pair) =>
        !(String(pair.foreground) in colors) ||
        !(String(pair.background) in colors) ||
        !Number.isFinite(pair.minimum) ||
        Number(pair.minimum) < 1,
    )
  )
    findings.push(
      finding(
  ...
  if (
    ...
    project.entry !== "src/render.ts" ||
    project.variantManifest !== "src/variants/manifest.json"
  )
    findings.push(
      finding(
        "PROJECT_INVALID",
...
function validateVariants(
  ...
  for (const [offset, variant] of variants.entries()) {
    ...
    if (
      !match ||
      variant.index !== offset + 1 ||
      Number(match.groups?.index) !== variant.index ||
    ...
    for (const [layerOffset, layer] of layers.entries()) {
      ...
      if (
        OWNER_VIOLATION.test(sourceText) ||
        (
          sourceText.match(
            /export\s+(?:async\s+)?function\s+buildLayer\s*\(/gu,
          ) ?? []
        ).length !== 1 ||
        /from\s+["']\.\/[0-9]{3}-[^"']+["']/u.test(sourceText)
      )
        findings.push(
          finding(
    ...
    if (stageAtLeast(stage, "render")) {
      for (const extension of ["svg", "png"] as const) {
        ...
        try {
          const inspection =
            extension === "svg"
              ? inspectPosterSvg(rawBytes(model, output))
              : inspectPosterPng(rawBytes(model, output));
...
function evidenceHeaderCurrent(
  model: PosterModel | null | undefined,
  evidence: JsonRecord,
  schema: string,
): boolean {
  return (
    ...
    evidence.plugin === "poster-production" &&
    evidence.artifactId === model?.artifactId &&
    evidence.subjectDigest === computePosterSubjectDigest(model) &&
...
function expectedRenderPaths(model: PosterModel | null | undefined): string[] {
  return [
    ...finalOutputPaths(model),
    ...Object.keys(model?.files ?? {}).filter((path) =>
      /^evidence\/layers\/.+\.(?:svg|png)$/u.test(path),
    ),
  ].sort();
}
...
function renderEvidenceCurrent(model: PosterModel | null | undefined): boolean {
  ...
  return (
    evidenceHeaderCurrent(model, evidence, RENDER_EVIDENCE_SCHEMA) &&
    ...
    outputs.every(
      (output, index) =>
        output.path === expected[index] &&
        output.sha256 === digestOf(model, expected[index] ?? ""),
    )
  );
}
...
function probeEvidenceCurrent(model: PosterModel | null | undefined): boolean {
  ...
  return (
    evidenceHeaderCurrent(model, evidence, PROBE_EVIDENCE_SCHEMA) &&
    ...
    measurements.every((measurement, index) => {
      ...
      return (
        measurement.id === id &&
        svg.width === variant.width &&
        svg.height === variant.height &&
        png.width === variant.width &&
        png.height === variant.height &&
        Number(png.alphaCoverage) >= 0.01 &&
    ...
    ["svg-png-byte-equivalence", "bounded-nonblank-raster"].every((criterion) =>
      checks.some(
...
function accessibilityEvidenceCurrent(
  model: PosterModel | null | undefined,
): boolean {
  ...
  return (
    evidenceHeaderCurrent(model, evidence, ACCESSIBILITY_EVIDENCE_SCHEMA) &&
    ...
    pairs.every((pair) =>
      checks.some(
        (check) =>
          check.criterion ===
            `contrast:${String(pair.foreground)}:${String(pair.background)}` &&
          Number(check.value) >= Number(pair.minimum),
      ),
    ) &&
    typographyRoles.every((role) =>
      checks.some((check) => check.role === role && check.status === "pass"),
    )
  );
}

function compositionEvidenceCurrent(
  model: PosterModel | null | undefined,
): boolean {
  ...
  return (
    ...
    measurements.every((measurement, index) => {
      ...
      return (
        ...
        Number.isFinite(foreground) &&
        Number.isFinite(voidCoverage) &&
...
function reviewEvidenceCurrent(model: PosterModel | null | undefined): boolean {
  ...
  return (
    evidenceHeaderCurrent(model, evidence, REVIEW_SCHEMA) &&
    reviewed.length === variants.length &&
    reviewed.every(
      (variant, index) =>
        variant.id === variants[index]?.id &&
...
export function validatePosterModel(
  model: PosterModel | null | undefined,
  { stage = "source" }: PosterValidateOptions = {},
): ContractFinding[] {
  ...
  if (stageAtLeast(typedStage, "review")) {
    if (!reviewEvidenceCurrent(model))
      ...
      if (
        ...
        !reviewer.sessionId ||
        reviewer.sessionId === renderSession ||
        reviewer.sessionId === (process.env.AI_EXPERTS_SESSION_ID || "unknown")
      )
        findings.push(
          finding(
            "REVIEW_SELF",
...
export function resolveWorkspaceRoot(cwd: string): string {
  ...
  while (current !== dirname(current)) {
    if (
      basename(dirname(current)) === "poster" &&
      basename(dirname(dirname(current))) === "artifacts"
    )
...
export function isPosterProjectRoot(
  root: string,
  workspaceRoot: string,
): boolean {
  return (
    dirname(resolve(root)) === resolve(workspaceRoot, "artifacts", "poster") &&
    /^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(basename(root))
  );
}
...
export async function findPosterProjects(cwd: string): Promise<string[]> {
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => join(carrier, entry.name))
      .sort();
  } catch (error: unknown) {
    if (
      typeof error === "object" &&
...
async function collectProject(
  root: string,
  directory: string,
  model: Required<Pick<PosterModel, "files" | "bytes" | "digests">>,
...
export function evaluatePosterWrite({
  relativePath = "",
  toolName = "",
  writer = "",
  cwd = "",
}: PosterWriteOptions = {}): PosterWriteDecision {
  const inside = posterProjectInside(relativePath, cwd);
  if (!inside) return { decision: "allow" };
  if (
    (GENERATED_PATH.test(inside) || SOURCE_PROOF_PATH.test(inside)) &&
    !writer.startsWith("poster-")
  )
    return {
      decision: "deny",
