...
export async function withWriterJournal<T>(root: string, capability: string, callback: () => Promise<T>, grant: WriterSessionGrant = {}) {
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schemaVersion: 2, plugin: "poster-production", operation: capability, artifactId: basename(root), ...sessionMetadata(capability, grant) })}\n`);
    await handle.sync();
