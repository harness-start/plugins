...
Create `poster-production/review-input/v2` JSON outside the project. Bind the current artifact id, subject digest, every variant id and PNG digest, your own session id, checks, findings, and verdict. Every finding has severity, exact visual anchor, concrete evidence, disposition, and recovery.
