...
function read(path: string): { text: string } | null { try { const bytes = readFileSync(path); return bytes.length <= 2 * 1024 * 1024 ? { text: bytes.toString("utf8") } : null; } catch { return null; } }
function count(text: string, predicate: (line: string) => boolean): number { return text.split("\n").filter(predicate).length; }
function testPath(path: string): boolean { const normalized = path.replaceAll("\\", "/"); return /\/(?:tests?|spec|__tests__|__mocks__|fixtures?|testdata|e2e)\//u.test(normalized) || /\.(?:test|spec|e2e)\.[^.]+$/u.test(basename(path)); }

export function fileSafetyReports(path: string, input: Record<string, unknown> = {}): string[] {
  const content = read(path); if (!content) return []; const extension = extname(path).toLowerCase(), reports: string[] = [];
