...
test("Codex parent-lane oracle accepts parent writes outside worker lanes", () => {
  assert.equal(parentAvoidedLanes([
    ...
    call("exec_command", "exec-absolute-workspace", {
      ...
      workdir: "/out/engineering-workflow__specification-04-multi-task-context-isolation__codex/workspace",
    }),
  ]), true);
  assert.equal(parentAvoidedLanes([
    {
  ...
  assert.equal(parentAvoidedLanes([
    call("exec_command", "exec-skill", {
      cmd: "cat /out/example/codex-home/plugins/cache/harness-start/engineering-workflow/1.0.0/skills/sdd-build/SKILL.md",
    }),
