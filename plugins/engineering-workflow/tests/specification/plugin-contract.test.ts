...
import { assertHandlerRoutedOnBothHosts, readHandlerRoutes } from "../../../../core/tests/support/aio-routes.js";

const ROOT = fileURLToPath(new URL("../..", import.meta.url));

test("owner routes the specification handler with five bundled skills and artifact-only hooks", () => {
  assertHandlerRoutedOnBothHosts(import.meta.url, "specification");
  const names = readdirSync(join(ROOT, "skills"), { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith("sdd")).map((entry) => entry.name).sort();
  assert.deepEqual(names, ["sdd", "sdd-build", "sdd-plan", "sdd-specify", "sdd-tasks"]);

  for (const host of ["claude", "codex"] as const) {
    const routes = readHandlerRoutes(import.meta.url, host, "specification");
...
test("plugin ships Docker acceptance cases for control, recovery, isolation, and resume", () => {
  for (const name of [
    ...
    "specification-02-artifact-order-recovery",
    "specification-03-stale-upstream-recovery",
    "specification-04-multi-task-context-isolation",
    "specification-05-resume-from-artifacts",
    "specification-06-review-and-scope-defense",
  ]) {
