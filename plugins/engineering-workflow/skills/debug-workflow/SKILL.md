...
Open a Debug Work Order with the owner CLI before changing production code. The writer is the activation signal for the debugging workflow; merely loading this Skill, or editing a markdown file, does not activate hooks.
...
```bash
DWG_PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}"
test -f "$DWG_PLUGIN_ROOT/dist/cli/harness.mjs"
DWG="$DWG_PLUGIN_ROOT/dist/cli/harness.mjs"
