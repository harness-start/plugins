---
name: sdd-specify-method
description: Specify-plan-tasks method used by this plugin's sdd Skill. Project artifacts stay under .specs/.
---
...
Execute tasks one at a time through this plugin's `$sdd-build` Skill. It owns the bounded task loop, public-seam RED requirement, parent diff review, and task-level verification. Load only the current task's spec sections and declared source files rather than flooding the agent with the entire spec.
