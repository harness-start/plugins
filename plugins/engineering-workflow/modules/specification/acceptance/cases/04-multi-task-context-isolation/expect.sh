...
jq -se -f "${ACCEPT_REPO}/plugins/spec-driven-development/acceptance/lib/codex-wait-receipt.jq" "${parent_transcript}" >/dev/null
...
if [[ -f "${ACCEPT_WORKSPACE}/verification.txt" ]]; then
  ...
  require_file_exists "${ACCEPT_WORKSPACE}/rejection.txt"
  test "$(cat "${ACCEPT_WORKSPACE}/rejection.txt")" = "parent rejected unverified workers"
  jq -se -f "${ACCEPT_REPO}/plugins/spec-driven-development/acceptance/lib/codex-parent-lane.jq" "${parent_transcript}" >/dev/null
  lane_side_effects=0
