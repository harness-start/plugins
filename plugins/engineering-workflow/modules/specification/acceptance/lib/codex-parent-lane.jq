def allowed_parent_call:
  if .payload.type == "function_call" and .payload.name == "spawn_agent" then true
    ...
    ((.payload.arguments | fromjson? // {}) as $arguments
      | (($arguments.cmd | test("^cat /out/[A-Za-z0-9._/-]+/spec-driven-development/[0-9]+\\.[0-9]+\\.[0-9]+/skills/sdd-build/SKILL\\.md(?:; echo '---'; ls -la /out/[A-Za-z0-9._/-]+/workspace)?$"))
      ...
      and (
        (($arguments.workdir // ".") == ".")
        or ($arguments.workdir == "/out/spec-driven-development__04-multi-task-context-isolation__codex/workspace")
      ))
