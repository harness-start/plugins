#!/usr/bin/env node
// harness-source-hash: sha256:79defd2d3595c36393e447d65453e58bc5d7f4a37d4d695b708964d11cb741cd
import "../chunks/chunk-AXBKD4XJ.mjs";
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-FZXEPJOZ.mjs");
...
// plugins/rust-engineering/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[rust-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
