...
function rustTargets(code, testPath) {
  const body = code.replace(/^\s*use\s+[^;]+;\s*$/gmu, "");
  const scope = rustScope(testPath, "tests");
  const targets = [];
  for (const match of code.matchAll(/^\s*use\s+([^;]+)\s*;/gmu)) {
    ...
    for (const path of paths) {
      ...
      segments.shift();
      targets.push(`rust:${scope}#${segments.join("::")}#${item}`);
    }
  }
  return unique(targets);
}
...
export function extractTestEvidence(language, text, testPath = "") {
  ...
  else if (language === "rust") targets = rustTargets(code, testPath);
...
function explicitSourceTargets(source) {
  ...
  if (source.language === "rust") {
    ...
    return symbols.map((symbol) => `rust:${descriptor.scope}#${descriptor.module}#${symbol}`);
  }
  if (source.language === "go") {
    const directory = posix.dirname(normalize(source.path));
    return symbols.map((symbol) => `go:${directory}#${symbol}`);
  }
  return [];
}
...
function goPackageMatches(source, testRecord) {
  ...
  for (const target of testRecord.evidence.targets ?? []) {
    const match = target.match(/^go-import:(.+)#([A-Za-z_][A-Za-z0-9_]*)$/u);
    if (match && (match[1] === sourceDirectory || match[1].endsWith(`/${sourceDirectory}`)) && symbols.has(match[2])) return true;
  }
  return false;
}
...
export function sourceAuthorizedByTest(source, testRecord) {
  ...
  if (explicitSourceTargets(source).some((target) => testTargets.has(target))) return true;
