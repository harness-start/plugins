...
插件不会自行启动测试进程；缺少明确退出状态或可识别测试结果时不会记作 RED/GREEN。测试文件仍须通过语言实体绑定或完整目录镜像指向实现文件。会话状态只保存 RED/GREEN 回执，写在当前工作目录的 `.tdd-guard/state/`。`.tdd-guard/.gitignore` 只忽略 `state/`，插件不会修改项目根目录的 `.gitignore`。
