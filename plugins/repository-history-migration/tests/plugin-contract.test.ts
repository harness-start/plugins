...
test("plugin exposes one migration skill and dual-platform source-protect Hooks", () => {
  const codex = json(".codex-plugin/plugin.json");
  const claude = json(".claude-plugin/plugin.json");

  assert.equal(codex.name, "repository-history-migration");
...
test("plugin publishes readable scripts and appears once in both marketplaces", () => {
  ...
  for (const path of [
    ...
    const entries = marketplace.plugins.filter(
      (entry) => entry.name === "repository-history-migration",
    );
