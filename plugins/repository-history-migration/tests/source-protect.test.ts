...
import { classifySourceProtectCommand } from "../src/source-protect.ts";

test("allows the plugin execute CLI and ordinary read-only git", () => {
  assert.equal(classifySourceProtectCommand("node dist/cli/git-history-migration-execute.mjs --source /src --target /dst"), null);
