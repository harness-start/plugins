...
Laravel hard guards for Claude Code and Codex: **deny** writes into
...
| --- | --- | --- |
| Protected paths | **deny** | `bootstrap/cache/*.php`（编译缓存）、`storage/framework/cache/`、`storage/framework/views/`（编译视图）、`storage/logs/`、`public/build/`（Vite 产物）、既有 `database/migrations/YYYY_MM_DD_HHMMSS_*.php`（只许新建迁移） |
