#!/usr/bin/env node
/**
 * laravel-runtime-guards PreToolUse entry.
 *
 * Denies writes into Laravel generated/runtime/build paths.
...
function collectTargets(toolName, toolInput, cwd) {
  ...
  if (command) {
    ...
    for (const m of command.matchAll(
      /(?:^|[\s'"])((?:\.\/)?(?:bootstrap\/cache|storage\/framework\/cache|storage\/framework\/views|storage\/logs|public\/build|database\/migrations)\/[^\s'";&|]+)/g,
    )) {
      targets.push(m[1]);
    }
    if (command.includes("***")) {
