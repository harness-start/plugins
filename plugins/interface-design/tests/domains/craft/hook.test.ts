...
import { runSession } from "../../../src/domains/craft/entries/hooks/interface-craft.js";

const HOOK = fileURLToPath(new URL("../../../dist/hooks/dispatcher.mjs", import.meta.url));

test("interface craft Hook remains an import-safe owner handler", () => {
  assert.equal(typeof runSession, "function");
});

function run(mode: string, event: Record<string, unknown>, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [HOOK, "codex", ({ "session-start": "SessionStart", pre: "PreToolUse", post: "PostToolUse", failure: "PostToolUseFailure", stop: "Stop", session: "SessionStart", prompt: "UserPromptSubmit", "user-prompt": "UserPromptSubmit", subagent: "SubagentStart", "subagent-stop": "SubagentStop" } as Record<string, string>)[mode] ?? mode], {
...
test("invalid hook JSON fails open", () => {
  const result = spawnSync(process.execPath, [HOOK, "codex", "PostToolUse"], { input: "{", encoding: "utf8" });
