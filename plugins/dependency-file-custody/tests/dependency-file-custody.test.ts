...
import {
  ...
  matchRule,
  resolveRules,
} from "../src/lib/protected-file-policy.js";
import {
  extractFileTargets,
  extractPatchTargets,
  matchPathsForTarget,
} from "../src/entries/hooks/dependency-file-custody.js";

const ENTRY = fileURLToPath(
  new URL("../dist/hooks/dependency-file-custody.mjs", import.meta.url),
);
...
test("project config can narrowly allow a built-in protected path", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(root, ".dependency-file-custody.mjs"),
      [
        "export default {",
        "  rules: [",
...
test("broken project config warns and keeps built-in protection", async () => {
  ...
  try {
    ...
    assert.equal(result.code, 0);
    assert.match(result.stderr, /failed to load \.dependency-file-custody\.mjs/u);
    assert.equal(
      JSON.parse(result.stdout).hookSpecificOutput.permissionDecision,
      "deny",
    );
