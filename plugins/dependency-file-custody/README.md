...
`dependency-file-custody` 在 Claude Code 和 Codex 的写入发生前检查目标路径，禁止直接创建、修改、移动或删除依赖 lockfile 和包管理器拥有的第三方依赖文件。
...
在 Git 项目根目录创建 `.dependency-file-custody.mjs`：
...
用户规则在内置规则之前执行，first-match-wins。`mode` 只能是 `block` 或 `allow`，缺省为 `block`。使用插件自带的 `dependency-file-custody-config` Skill 初始化、维护或诊断配置。
...
配置按 `.dependency-file-custody.mjs`、`.dependency-file-custody.cjs`、`.dependency-file-custody.js` 顺序从事件工作目录对应的 Git 根通过 `import()` 加载。非 Git 目录仍使用内置规则，但不加载项目配置。配置损坏时写一行警告并保留内置保护；非法单条规则只警告并跳过；Hook 输入畸形或意外运行错误时 fail-open。
...
```bash
# Claude Code
claude plugin install dependency-file-custody@harness-start

# Codex
codex plugin add dependency-file-custody@harness-start
```
...
```bash
npx tsx --test plugins/dependency-file-custody/tests/*.test.ts
