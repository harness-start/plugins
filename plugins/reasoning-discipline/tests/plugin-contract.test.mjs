...
test("plugin publishes two focused skills without the legacy workflow machinery", () => {
  ...
  assert.equal(codex.name, "reasoning-discipline");
  ...
  const skillNames = readdirSync(join(ROOT, "skills"), { withFileTypes: true })
    ...
    .sort();
  assert.deepEqual(skillNames, ["first-principles", "reasoning-discipline"]);
  ...
  for (const skillName of skillNames) {
    ...
    assert.doesNotMatch(skill, /\.reasoning-discipline|\.first-principles|ledger|workflow\.md|completionReceipt/iu);
  }

  for (const path of ["scripts", "hooks", ".reasoning-discipline", ".first-principles"]) {
...
test("skills select an adaptive reasoning structure and a direct evidence-bound output", () => {
  ...
  const discipline = text("skills/reasoning-discipline/SKILL.md");
...
test("both marketplaces replace the two legacy plugin ids with the merged plugin", () => {
  for (const legacyName of ["first-principles-gate", "reasoning-discipline-guard"]) {
