...
test("resolveConfig rejects unsafe auditRoot", () => {
  const warnings = [];
  assert.equal(resolveConfig({ auditRoot: "../escape" }, (m) => warnings.push(m)).auditRoot, ".agent-activity-audit");
  assert.equal(resolveConfig({ auditRoot: "src" }, (m) => warnings.push(m)).auditRoot, ".agent-activity-audit");
  assert.equal(resolveConfig({ auditRoot: "/abs" }, (m) => warnings.push(m)).auditRoot, ".agent-activity-audit");
...
test("shell protect allows node without audit root and denies /bin/rm of trail", () => {
  ...
  const abs = "/tmp/p/.agent-activity-audit";
  ...
  assert.equal(
    shellMutatesAuditRoot(
      "python3 -c \"open('.agent-activity-audit/sessions/x.jsonl','w').write('x')\"",
      rel,
      abs,
    ),
    true,
...
test("pre denies interpreter rewrite of the file-access trail", async () => {
  ...
  try {
    const deny = await runEntry("pre", {
      ...
      tool_name: "Bash",
      tool_input: {
        command: "node -e \"require('fs').writeFileSync('.agent-activity-audit/sessions/s.jsonl','x')\"",
      },
    });
...
test("post records edit with a local gitignore without modifying the project gitignore", async () => {
  ...
  try {
    ...
    assert.equal(lines[0].schema, "agent-activity/v1");
...
test("pre denies edit of audit session file", async () => {
  const root = workspace();
  try {
    mkdirSync(join(root, ".agent-activity-audit", "sessions"), { recursive: true });
    writeFileSync(join(root, ".agent-activity-audit", "sessions", "sess.jsonl"), "{}\n");
    const result = await runEntry("pre", {
      cwd: root,
      session_id: "sess",
      tool_name: "Edit",
      tool_input: {
        file_path: ".agent-activity-audit/sessions/sess.jsonl",
        old_string: "{}",
        new_string: "x",
      },
    });
