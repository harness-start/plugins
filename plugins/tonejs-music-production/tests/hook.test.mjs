...
const ENTRY = fileURLToPath(new URL("../scripts/tonejs-music-production.mjs", import.meta.url));
...
test("denies a shell payload that merely smuggles a registered wrapper path as an argument", async () => {
  const result = await runPre("node -e \"require('fs').writeFileSync('artifacts/music/study/dist/forged.wav','x')\" plugins/tonejs-music-production/scripts/tools/project-render.mjs artifacts/music/study");
  assert.equal(result.code, 0);
  assert.equal(JSON.parse(result.stdout).hookSpecificOutput.permissionDecision, "deny");
});
