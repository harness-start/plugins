...
`tonejs-music-production` 同时支持 Codex 和 Claude。旋律、和声、节奏、曲式和乐器写在可审查的 JavaScript 模块里；有限、确定的数学搜索排出候选乐谱，再用浏览器里的 `Tone.Offline` 离线渲染。源码绑定、客观音频分析和当次人工听审都过了，才允许发布 48 kHz 双声道 PCM16 WAV。
...
插件要能核对这条链路：源码决定乐谱，优化器排出候选，Tone.js 合成，音频分析抓可测问题，人听完再判断是否达标。
