// plugins/tonejs-music-production/src/lib/contract.ts
import { createHash } from "node:crypto";
var GENERATED_PATH = /^(?:build\/|proofs\/|dist\/|evidence\.audio\.json$|release\.manifest\.json$|receipt\.release\.json$)/u;
var SUBJECT_EXCLUDED_PATH = /^(?:plan\.contract\.json$|build\/|proofs\/|dist\/|evidence\.audio\.json$|release\.manifest\.json$|receipt\.release\.json$|review\/|\.music-delivery-journal\.json$)/u;
...
var MUSIC_ENGINE = "tonejs-music-production@0.2.0";
...
function validateMusicReceipt(model) {
  try {
    ...
    return actual?.schemaVersion === expected.schemaVersion && actual?.plugin === expected.plugin && actual?.engine === expected.engine && actual?.artifactId === expected.artifactId && actual?.stage === expected.stage && actual?.subjectDigest === expected.subjectDigest && JSON.stringify(actual?.outputs) === JSON.stringify(expected.outputs);
  } catch {
    return false;
  }
}
...
function validateSourceArtifacts(model, findings) {
  ...
  if (renderReceipt && (renderReceipt.schema !== "tonejs-render-receipt/v1" || renderReceipt.sourceDigest !== paths.subjectDigest || JSON.stringify(renderReceipt.outputs) !== JSON.stringify(expectedOutputs))) {
...
function evaluateMusicWrite({ relativePath = "", toolName = "", writer = "" } = {}) {
