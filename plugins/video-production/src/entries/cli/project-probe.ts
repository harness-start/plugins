...
function parseProjectJson(model: VideoModel, path: string) {
  try { return JSON.parse(model.files?.[path] ?? "null") as unknown; } catch { return null; }
}
...
async function main() {
  ...
  const motionBeats = beats.map((beat) => {
    const start = Number(beat.startFrame);
    const end = Number(beat.endFrame);
    const frames = [...new Set([start, Math.floor((start + end - 1) / 2), end - 1])];
    const beatSamples = frames.map((frame) => sampleMap.get(frame)).filter((sample) => sample !== undefined);
  ...
  for (const reference of referenceList) {
    if (reference.fidelity !== "frame-aligned") { comparisons.push({ id: String(reference.id), fidelity: reference.fidelity, verdict: "review" }); continue; }
