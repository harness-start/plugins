...
export function writeJson(value) { if (value === null) return; process.stdout.write(`${JSON.stringify(value)}\n`); }
...
export function report(eventName, text) { const output = { hookSpecificOutput: { hookEventName: eventName, additionalContext: text } }; if (process.env.PLUGIN_ROOT && eventName === "PostToolUse") { process.stderr.write(`${text}\n`); process.exitCode = 2; return null; } return output; }
