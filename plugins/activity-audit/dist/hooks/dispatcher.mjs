// harness-source-hash: sha256:d36db61e7b9155cf134138a014848547faeb893d870b99388a250e4e6260f179
...
// plugins/activity-audit/src/domains/activity/entries/hooks/agent-activity-audit.ts
...
// plugins/activity-audit/src/domains/activity/lib/hook-io.ts
...
// plugins/activity-audit/src/domains/activity/lib/command-policy.ts
...
function inferCommandStatus(event, forceFailure = false) {
  ...
  if (typeof response === "string") {
    ...
    const codeText = matches2.at(-1)?.[1];
    if (codeText !== void 0) {
...
// plugins/activity-audit/src/domains/activity/lib/config.ts
...
// plugins/activity-audit/src/domains/activity/lib/jsonl-trail.ts
...
function trailPaths(repoRoot, auditRoot, sessionKey) {
  const root = join3(resolve3(repoRoot), auditRoot);
  return {
    root,
...
function acquireLock(sessionPath) {
  ...
  mkdirSync2(dirname2(sessionPath), { recursive: true, mode: 448 });
...
// plugins/activity-audit/src/domains/activity/lib/jsonl-trail.ts
var README_TEXT = `# Agent activity audit

Append-only JSONL trail of structured file access and agent shell commands (one file per session).
...
function findPendingByToolUseId(sessionPath, toolUseId) {
  ...
  const content = readFileSync4(sessionPath, "utf8");
  let found = null;
  for (const line of content.split("\n")) {
    if (!line.trim()) continue;
    try {
      const parsed = JSON.parse(line);
      if (isRecord(parsed) && parsed.schema === "agent-activity/v1" && parsed.kind === "command" && parsed.status === "pending" && String(parsed.tool_use_id ?? "") === id) {
        found = parsed;
      }
    } catch {
    }
...
// plugins/activity-audit/src/domains/activity/lib/paths.ts
import { execFileSync } from "node:child_process";
import { isAbsolute as isAbsolute3, relative, resolve as resolve4 } from "node:path";
...
// plugins/activity-audit/src/domains/activity/lib/protect.ts
import { realpathSync } from "node:fs";
import { basename, dirname as dirname3, resolve as resolve6 } from "node:path";
...
function pathUnderRoot(filePath, rootAbs) {
  const rel = relative2(resolve5(rootAbs), resolve5(filePath)).replaceAll("\\", "/");
...
function canonicalPath(path) {
  ...
  while (true) {
    try {
      return resolve6(realpathSync(cursor), ...suffix);
    } catch {
    }
...
function finalizeRecord(base, event, forceFailure, config, now = /* @__PURE__ */ new Date()) {
  ...
  return {
    ...
    cwd: stringField(base, "cwd") ?? resolve7(eventCwd(event)),
...
function matchingPendingTip(sessionPath, toolUseId) {
  ...
  try {
    ...
    if (isRecord(parsed) && parsed.schema === "agent-activity/v1" && parsed.kind === "command" && parsed.status === "pending" && sameToolUseId(parsed.tool_use_id, id)) {
      return parsed;
    }
  } catch {
    return null;
...
async function main() {
  ...
  const auditRootAbs = resolve7(repoRoot, config.auditRoot);
  const toolName = eventToolName(event);
  if (mode === "pre") {
  ...
  try {
    ...
    if (tipBase) {
      ...
      const result = rewriteTip(
        paths.sessionPath,
        (parsed) => isRecord(parsed) && parsed.schema === "agent-activity/v1" && parsed.kind === "command" && parsed.status === "pending" && sameToolUseId(parsed.tool_use_id, tipBase.tool_use_id),
        finalRecord2
      );
...
// plugins/activity-audit/src/entries/hooks/dispatcher.ts
...
await runOwnerDispatcher(host, eventName, {
  "activity:agent-activity-audit": ownerHookHandler(main)
});
