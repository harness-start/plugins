...
import { evaluateStop } from "../src/entries/hooks/evidence-based-research.js";
import { appendStateEvent, readState } from "../src/lib/state-store.js";
import { defaultWorkflow, ensureRunSkeleton, readWorkflowFile, workflowPath, writeWorkflow } from "../src/lib/workflow-fs.js";
...
function runHook(mode, event, dataRoot) {
  const script = fileURLToPath(new URL("../dist/hooks/evidence-based-research.mjs", import.meta.url));
...
test("sealed evidence remains reusable across outbound and implementation mutations", async () => {
  ...
  const lifecycle = runHook("post", {
    ...event,
    tool_name: "exec_command",
    tool_input: { cmd: `node "${pluginRoot}/dist/cli/research-workflow.mjs" handoff-outbound --cwd "${workspace}" --handoff-file /tmp/h --prompt-file /tmp/p` },
  }, dataRoot);
  ...
  const chained = runHook("post", {
    ...event,
    tool_name: "exec_command",
    tool_input: { cmd: `node "${pluginRoot}/dist/cli/research-workflow.mjs" handoff-outbound --cwd "${workspace}" --handoff-file /tmp/h --prompt-file /tmp/p; echo changed > notes.md` },
  }, dataRoot);
