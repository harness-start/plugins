...
`video-project-delivery-guard` 为 `artifacts/video/<video-id>/` 提供一个统一的 Remotion 视频生产编排入口、一个只读独立审查入口，以及由摘要绑定的 init → plan → admit → compose → render → probe → review → release 交付闭环。
...
```bash
node plugins/video-project-delivery-guard/dist/cli/project-init.mjs artifacts/video/demo --profile motion-explainer --mode guided
...
```bash
node plugins/video-project-delivery-guard/dist/cli/project-admit.mjs artifacts/video/demo /tmp/external-run.json
```
