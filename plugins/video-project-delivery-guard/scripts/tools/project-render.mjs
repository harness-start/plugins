...
import { audioProofPaths, createVideoRenderProof, finalRenderPaths, validateVideoModel, visualProofPaths } from "../lib/contract.mjs";
import { consumeWriterCapability, processWriterArgv } from "../lib/capability.mjs";
import { probeMedia, validateMeasuredMedia } from "../lib/media.mjs";
...
function runNpm(root, script, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => { clearTimeout(timer); if (settled) return; settled = true; code === 0 ? resolve() : reject(new Error(`RENDER_SCRIPT_FAILED:${script}:${stderr.trim()}`)); });
  });
}
