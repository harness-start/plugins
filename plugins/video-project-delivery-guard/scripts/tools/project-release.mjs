...
import { createVideoReceipt, validateVideoModel, validateVideoReceipt } from "../lib/contract.mjs";
...
async function load() {
  ...
  const parse = (filePath) => { try { return JSON.parse(files[filePath] ?? ""); } catch { return null; } };
  return { artifactId: basename(root), files, digests, plan: parse("plan.contract.json"), project: parse("video.project.json") };
}

async function main() {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(basename(root))) throw new Error("project root must end in a kebab-case artifact id");
  const model = await load();
  const findings = validateVideoModel(model, { stage: "release" }).filter(({ code, path }) => code !== "RECEIPT_INVALID" && !(code === "RELEASE_PATH_MISSING" && path === "receipt.release.json"));
  if (findings.length > 0) throw new Error(findings.map(({ code, path }) => `${code}:${path}`).join(", "));
  const handle = await open(journalPath, "wx");
  await handle.writeFile(`${JSON.stringify({ schemaVersion: 1, plugin: "video-project-delivery-guard", operation: "release", artifactId: basename(root), sessionId: process.env.AI_EXPERTS_SESSION_ID ?? "unknown" })}\n`);
  await handle.sync();
  await handle.close();
  let complete = false;
  try {
    const receipt = { ...createVideoReceipt(model), createdAt: new Date().toISOString(), sessionId: process.env.AI_EXPERTS_SESSION_ID ?? "unknown", triggerFrom: process.env.AI_EXPERTS_TRIGGER_FROM ?? "unknown" };
    await writeFile(temporaryPath, `${JSON.stringify(receipt, null, 2)}\n`, { flag: "wx" });
    await rename(temporaryPath, receiptPath);
    if (!validateVideoReceipt(await load())) throw new Error("written receipt did not verify against current files");
    complete = true;
