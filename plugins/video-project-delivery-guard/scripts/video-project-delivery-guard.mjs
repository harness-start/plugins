...
async function discover(cwd) {
  const root = join(cwd, "artifacts", "video");
  try {
...
async function findingsFor(cwd) {
  ...
  for (const root of await discover(cwd)) {
    ...
    try { project = JSON.parse(files["video.project.json"]); } catch {}
    const model = { artifactId: basename(root), files, digests, plan, project };
    for (const item of validateVideoModel(model, { stage: plan?.targetStage ?? "source" })) findings.push({ artifactId: model.artifactId, ...item });
  }
  return findings;
}

function format(findings) {
  return ["[Video Project Delivery Guard] Project contract violations", ...findings.slice(0, 50).map((item) => `- ${item.artifactId}/${item.path} [${item.code}] ${item.message}`), "recovery: Fix the named variant, layer, proof, or output and rerun the registered video tool."].join("\n");
}
...
async function main() {
  ...
  if (mode === "pre") {
    for (const target of targetsOf(event)) {
      const result = evaluateVideoWrite({ relativePath: relative(cwd, resolve(cwd, target)), toolName: nameOf(event) });
    ...
    const cwdInScope = /(?:^|[\\/])artifacts[\\/]video[\\/][^\\/]+(?:[\\/]|$)/u.test(cwd);
    const mutates = (/artifacts[\\/]video[\\/]/u.test(command) || cwdInScope) && (/[>]{1,2}/u.test(command) || /(?:^|\s)(?:cp|mv|rm|touch|tee|node|npm|npx|python\d*)\b/u.test(command));
    const compoundShell = /(?:&&|\|\||[;&|><`\n]|\$\()/u.test(command);
    const approvedWrapper = /video-project-delivery-guard[\\/]scripts[\\/]tools[\\/](?:project-lint|project-release)\.mjs\b/u.test(command) && !compoundShell;
    if (mutates && !approvedWrapper) process.stdout.write(`${JSON.stringify(deny("UNKNOWN_MUTATION_SHELL: video mutations require a registered wrapper"))}\n`);
    return;
  }
  const findings = await findingsFor(cwd);
  if (mode === "session") {
    if ((await discover(cwd)).length > 0) process.stdout.write(`${JSON.stringify(context("SessionStart", "[Video Project Delivery Guard] active; generated outputs require registered writers."))}\n`);
