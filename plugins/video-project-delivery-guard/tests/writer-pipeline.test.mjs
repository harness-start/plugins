...
import { loadVideoProject } from "../scripts/lib/project.mjs";
import { validateVideoModel } from "../scripts/lib/contract.mjs";
import { withWriterJournal } from "../scripts/lib/writer.mjs";

const TOOLS = fileURLToPath(new URL("../scripts/tools/", import.meta.url));
const HOOK = fileURLToPath(new URL("../scripts/video-project-delivery-guard.mjs", import.meta.url));
...
function fixture(sandbox) {
  ...
  const files = {
    ...
    "video.project.json": JSON.stringify({ artifactId: "demo", durationInFrames: 10, fps: 30, width: 1920, height: 1080, compositionId: "Main" }),
  ...
  write(bin, "ffmpeg", "#!/usr/bin/env node\nprocess.stdout.write(Buffer.from(`FRAME:${process.argv.join(' ')}`));\n", true);
...
test("registered writers produce a structured render-probe-review-release closure", async () => {
  ...
  try {
    ...
    writeFileSync(reviewInput, JSON.stringify({
      ...
      frames: [0, 5, 9],
      checks: { captionsReviewed: true, flashingReviewed: true, contrastReviewed: true },
    ...
    assert.equal(JSON.parse(model.files["receipt.release.json"]).schemaVersion, 2);
...
test("review writer rejects a self-review from the release session", async () => {
  ...
  try {
    ...
    writeFileSync(input, JSON.stringify({ schema: "video-project-delivery-guard/review-input/v1", artifactId: "demo", outputSha256: sha256("FAKE_FINAL_MP4"), verdict: "pass", reviewer: { kind: "independent-agent", id: "forged-other-session", sessionId: "forged-session" }, frames: [0, 5, 9], checks: { captionsReviewed: true, flashingReviewed: true, contrastReviewed: true } }));
