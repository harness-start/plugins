...
function fixture(sandbox) {
  ...
  const direction = JSON.stringify({ schema: "video-project-delivery-guard/direction/v1", motionThesis: "A signal becomes a resolved outcome.", visualMetaphor: "signal path", narrativeArc: "hook, mechanism, resolution", motionGrammar: ["route", "resolve"], negativeRules: ["slide-deck pacing"] });
  const script = JSON.stringify({ schema: "video-project-delivery-guard/script/v1", beats: [{ id: "demo", narration: "A short demonstration." }], claims: [] });
  const storyboard = JSON.stringify({ schema: "video-project-delivery-guard/storyboard/v2", beats: [{ index: 1, id: "demo", startFrame: 0, endFrame: 10, narrativeJob: "demonstrate", movingObject: "signal", stateChange: "moves across the frame", cameraMotion: "stable", textRole: "label", assetIds: ["bed"], pptRisk: "static card" }] });
  const assets = JSON.stringify({ schema: "video-project-delivery-guard/assets/v2", assets: [{ id: "bed", kind: "audio", source: "user", path: "public/audio/bed.wav", rights: "owned" }] });
  const workers = [
  ...
  const files = {
    ...
    "plan.contract.json": JSON.stringify({ schema: "video-project-delivery-guard/plan/v2", artifactId: "demo", profile: "motion-explainer", mode: "guided", targetStage: "release", audience: "test viewers", objective: "verify the delivery closure", platform: "test", language: "en", assumptions: [], externalBudget: { currency: "USD", limit: 0, spent: 0 } }),
    "plan.direction.json": direction,
    "plan.script.json": script,
    "plan.storyboard.json": storyboard,
    "plan.skill-composition.json": JSON.stringify({ schema: "video-project-delivery-guard/skill-composition/v1", workers }),
    "plan.assets.json": assets,
    "plan.approvals.json": JSON.stringify({ schema: "video-project-delivery-guard/approvals/v1", mode: "guided", gates: [{ stage: "direction", status: "approved", subjectSha256: sha256(direction), actor: "fixture", reason: "" }, { stage: "storyboard", status: "approved", subjectSha256: storyboardDigest, actor: "fixture", reason: "" }, { stage: "assets", status: "approved", subjectSha256: sha256(assets), actor: "fixture", reason: "" }] }),
    "plan.references.json": JSON.stringify({ schema: "video-project-delivery-guard/references/v1", references: [] }),
    "design.system.json": JSON.stringify({ schema: "video-project-delivery-guard/design-system/v1", colors: { canvas: "#111111", text: "#ffffff", accent: "#5eead4" }, typography: { displayPx: 96, bodyPx: 42, captionPx: 36 }, safeAreaPx: 80, motion: { enterFrames: 3, exitFrames: 3, easing: "ease-out" }, captions: { maxCharsPerSecond: 20, maxLines: 2 }, audio: { integratedLufs: -16, truePeakDb: -1 } }),
    "video.project.json": JSON.stringify({ schema: "video-project-delivery-guard/project/v2", artifactId: "demo", profile: "motion-explainer", durationInFrames: 10, fps: 30, width: 1920, height: 1080, compositionId: "Main" }),
...
test("registered writers produce a structured render-probe-review-release closure", async () => {
  ...
  try {
    ...
    writeFileSync(reviewInput, JSON.stringify({
      schema: "video-project-delivery-guard/review-input/v2",
      artifactId: "demo",
    ...
    assert.equal(JSON.parse(model.files["evidence.probe.json"]).schema, "video-project-delivery-guard/probe-evidence/v1");
