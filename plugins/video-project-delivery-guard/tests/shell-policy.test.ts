...
import { evaluateVideoShell } from "../src/lib/shell-policy.js";
...
test("allows only an exact installed-plugin writer invocation", () => {
  ...
  try {
    const result = evaluateVideoShell({
      command: 'node "${PLUGIN_ROOT}/dist/cli/project-render.mjs" artifacts/video/demo final',
      cwd: "/workspace",
...
test("rejects compounds even when they contain an exact writer path", () => {
  const result = evaluateVideoShell({
    command: `node "${PLUGIN_ROOT}/dist/cli/project-render.mjs" artifacts/video/demo final ; node -e "0"`,
    cwd: "/workspace",
    workspaceRoot: "/workspace",
  });
