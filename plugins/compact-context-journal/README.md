...
`compact-context-journal` 在项目里记一份会话日志，方便上下文被压缩后把已经确认的需求找回来。日志能恢复，不保证模型一定读对。

硬规则是：只追加、校验完整性、恢复上下文有上限、改文件前必须先成功读过 Recovery Card，以及工具层不许改这份日志。
...
模型不会自动拿到整份原始历史，这样能少带过期权限、少吃 prompt injection、也少涨上下文。恢复还是靠模型自己读卡片，所以插件只保证“找得回来”和“先读后写”，不保证任务一定做对。
