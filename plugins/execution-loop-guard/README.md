...
成功的测试、lint、typecheck、build 或其他明确验证命令会清空当前 session/workspace 的全部编辑计数，代表本轮修改已经产生可验证进展。插件状态写在当前工作目录的 `.execution-loop-guard/state/`；`.execution-loop-guard/.gitignore` 忽略该工作目录的全部内容，插件不会修改项目根目录的 `.gitignore`。状态不保存命令输出或文件内容。
