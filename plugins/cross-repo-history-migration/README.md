...
`cross-repo-history-migration` 提供一个公开 Skill 和两个确定性 CLI：先只读预检并密封 source head 与计划摘要，再在目标父目录的临时 clone 中执行过滤，最后原子发布为新的本地 Git 仓库。
...
完整用法见 [`skills/cross-repo-history-migration/SKILL.md`](skills/cross-repo-history-migration/SKILL.md)。
