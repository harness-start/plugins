...
1. Freeze `plan.brief.json`, `plan.context.json`, `plan.contract.json`, parsed asset/reference provenance, delivery profile, Figma capability/fallback, and the dynamic role-based bilingual worker selection. The brief binds exact `brandName` and `wordmarkText`, script/case policy, and the letterform type class, stroke profile, structure, edge finish, and scene reference.
