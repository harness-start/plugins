...
Keep `plan.brief.json` decision-complete and record all four current-source candidates in `plan.skill-composition.json`. At most three may be `used`. Every used worker needs an admitted, digest-bound `evidence/skills/<name>.json`; skipped and unavailable workers need an honest reason.
