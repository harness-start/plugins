...
Create an external JSON object with schema `brand-logo-production/review-input/v1`. Bind `artifactId`, current `subjectDigest`, every required artifact path and SHA-256, and a reviewer whose session is the current independent session.
