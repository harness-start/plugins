...
export type LogoStage = "source" | "release";

export type LogoValidateOptions = {
  stage?: unknown;
};

export type LogoWriteOptions = {
  relativePath?: string | undefined;
  toolName?: string | undefined;
};

export type LogoWriteDecision =
  | { decision: "allow" }
  | { decision: "deny"; code: string; message: string };

export type SvgCircle = { cx: number; cy: number; r: number };
...
export type LogoReleaseManifest = {
  schema: string;
  plugin: string;
  artifactId: string | undefined;
  subjectDigest: string;
...
export type ConstructionSheetFiles = Record<string, { path: string; sha256: string }>;
...
export type ConstructionManifest = {
  ...
  artifactId: string | undefined;
  masterDigest: string;
  sheets: Record<string, ConstructionSheetFiles>;
};
...
export type PreviewSample = {
  ...
  locator?: PreviewLocator | undefined;
};

const PATH_ARITY = { A: 7, C: 6, H: 1, L: 2, M: 2, Q: 4, S: 4, T: 2, V: 1, Z: 0 } as const;
...
export const PROJECT_SCHEMA = "brand-logo-production/project/v1";
export const CONSTRUCTION_SCHEMA = "brand-logo-production/construction/v1";
export const STANDARD_GRID_SCHEMA = "brand-logo-production/standard-grid/v1";
export const GEOMETRY_SCHEMA = "brand-logo-production/geometry/v1";
export const FIBONACCI_SCHEMA = "brand-logo-production/fibonacci/v1";
...
const rawBytes = (model: LogoModel | null | undefined, filePath: string): Buffer => {
...
const fileDigest = (model: LogoModel | null | undefined, filePath: string): string => model?.digests?.[filePath] ?? sha256(rawBytes(model, filePath));
const hasFile = (model: LogoModel | null | undefined, filePath: string): boolean => Object.prototype.hasOwnProperty.call(model?.files ?? {}, filePath);

function stageName(value: unknown): value is string {
  return typeof value === "string" && STAGES.has(value);
}

function parseJson(files: FileMap | undefined, filePath: string, findings: ContractFinding[], code = "JSON_INVALID"): unknown {
  ...
  try { return JSON.parse(textOf(files?.[filePath])) as unknown; } catch {
    findings.push(finding(code, filePath, `${filePath} must contain valid JSON`));
    return null;
  }
}

function generatedSubjectPath(filePath: string): boolean {
  return GENERATED_PATH.test(filePath) || CONCEPT_PROOF_PATH.test(filePath);
}

export function computeLogoSubjectDigest(model: LogoModel | null | undefined): string {
...
export function masterSubjectDigest(model: LogoModel | null | undefined): string {
...
function conceptPreviewPaths(model: LogoModel | null | undefined): string[] {
  try {
    const manifest: unknown = JSON.parse(textOf(model?.files?.["src/concepts/manifest.json"]));
    const concepts = isObject(manifest) && Array.isArray(manifest.concepts) ? manifest.concepts : [];
    return concepts.flatMap((entry: unknown) => {
      const record = isObject(entry) ? entry : undefined;
      const sourcePath = `src/concepts/${record?.source ?? ""}`;
      return hasFile(model, sourcePath) ? [`src/concepts/${(record?.source as string).slice(0, -9)}.${fileDigest(model, sourcePath)}.png`] : [];
    });
  } catch { return []; }
}
...
function finalOutputPaths(): string[] {
  return [
...
function previewEvidencePaths(model: LogoModel | null | undefined): string[] {
  const digest = masterSubjectDigest(model);
  return [
...
export function createLogoReleaseManifest(model: LogoModel | null | undefined): LogoReleaseManifest {
...
function exactJson(actualText: unknown, expected: unknown): boolean {
  try { return JSON.stringify(JSON.parse(textOf(actualText))) === JSON.stringify(expected); } catch { return false; }
}

export function validateLogoReceipt(model: LogoModel | null | undefined): boolean {
  try {
...
export function extractSvgCircles(svg: unknown): SvgCircle[] {
...
export function extractSvgPathPoints(svg: unknown): SvgPoint[] {
  if (typeof svg !== "string") return [];
...
function fibAdjacent(a: number, b: number): boolean {
...
function expectedRatio(larger: number, smaller: number): number | null {
...
function validatePlanAndProject(model: LogoModel, stage: unknown, findings: ContractFinding[]): void {
  ...
  const planRecord = isObject(plan) ? plan : undefined;
  ...
  else if ((isObject(plan) ? plan.targetStage : undefined) !== stage) findings.push(finding("PLAN_STAGE_MISMATCH", "plan.contract.json", "validated closure stage must match plan targetStage"));
  const project = parseJson(model.files, "logo.project.json", findings);
  const projectRecord = isObject(project) ? project : undefined;
...
function validateToolchain(files: FileMap, findings: ContractFinding[]): void {
  const pkg = parseJson(files, "package.json", findings);
  const lock = parseJson(files, "package-lock.json", findings);
  const scripts = isObject(pkg) && isObject(pkg.scripts) ? pkg.scripts : undefined;
...
function pngValid(model: LogoModel, filePath: string): boolean {
...
function svgAttributes(text: string): Map<string, string> {
...
function finiteNumber(value: unknown, { positive = false }: { positive?: boolean } = {}): boolean {
  if (typeof value !== "string" || !value.trim()) return false;
  const number = Number(value);
  return Number.isFinite(number) && (!positive || number > 0);
}

function isPathCommand(value: string): value is PathCommand {
  return Object.prototype.hasOwnProperty.call(PATH_ARITY, value);
}

function pathDataValid(value: unknown): boolean {
  if (typeof value !== "string" || !value.trim()) return false;
  ...
  while (cursor < tokens.length) {
    ...
    if (head !== undefined && commandPattern.test(head)) {
    ...
    while (cursor < tokens.length) {
      ...
      if (token === undefined || commandPattern.test(token)) break;
    ...
    if (command === "A") for (let offset = 0; offset < values.length; offset += size) {
      ...
      if (!(rx !== undefined && rx > 0) || !(ry !== undefined && ry > 0) || largeArc === undefined || sweep === undefined || ![0, 1].includes(largeArc) || ![0, 1].includes(sweep)) return false;
    }
...
function svgValid(value: unknown, { sheet = null, masterDigest = null }: { sheet?: string | null; masterDigest?: string | null } = {}): boolean {
...
function svgPrimitiveIds(value: unknown): Set<string> {
...
function geometrySignature(value: unknown): string {
  ...
  const elements = [...svg.matchAll(/<(svg|g|path|circle|ellipse|rect|line|polyline|polygon)\b([^>]*)>/gu)].map((match) => {
    ...
    const attributes = [...rawAttrs.matchAll(/\b([A-Za-z_:][-A-Za-z0-9_:.]*)\s*=\s*["']([^"']*)["']/gu)]
      .filter((attribute) => {
        const name = attribute[1];
        return name !== undefined && !["xmlns", "fill", "color"].includes(name) && !name.startsWith("aria-") && !name.startsWith("data-");
      })
      .map((attribute) => {
        const name = attribute[1] ?? "";
 ...
 */
type DeclaredCircle = { id: string; cx: number; cy: number; radiusUnits: number; r: number };

export function validateFibonacciConstruction(model: LogoModel, findings: ContractFinding[]): void {
  ...
  if (!new Set(["structural", "optical-reference"]).has(fib.usage as string)) {
  ...
  const byId = new Map<string, DeclaredCircle>();
  ...
  if (!spiral || spiral.kind !== "fibonacci-quarter-arcs" || !orderedIds || orderedIds.length < 3) {
    ...
    const ordered = orderedIds.map((id) => byId.get(id as string)).filter((value): value is DeclaredCircle => Boolean(value));
    if (ordered.length >= 2) {
      let maxCenterDist = 0;
    ...
    for (let i = 0; i < orderedIds.length - 1; i += 1) {
      const a = byId.get(orderedIds[i] as string);
      const b = byId.get(orderedIds[i + 1] as string);
      if (!a || !b) {
    ...
    if (Array.isArray(spiral.arcs) && spiral.arcs.length > 0) {
      for (const arc of spiral.arcs) {
        ...
        if (!byId.has(arcRec?.circleId as string)) {
        ...
        if (Math.abs(norm - 90) > 1 && Math.abs(norm - 270) > 1) {
          findings.push(finding("FIBONACCI_SPIRAL_ARC_INVALID", path, `spiral arc on ${arcRec?.circleId} must be a quarter turn (90°), got ${sweep}°`));
        }
      }
    }
  }
  ...
  for (const binding of bindings) {
    const item = rec(binding);
    const circle = byId.get(item?.circleId as string);
    if (!circle) continue;
...
function validateConstruction(model: LogoModel, findings: ContractFinding[]): void {
  ...
  const mappingPrimitives = new Map<string, Set<unknown>>();
  for (const mapping of mappings) {
    ...
    if (typeof item?.role !== "string" || !ROLES.includes(item.role) || typeof item?.pathId !== "string" || !masterIds.has(item.pathId) || !Array.isArray(item?.primitiveIds) || primitiveIdList.length === 0 || new Set(primitiveIdList).size !== primitiveIdList.length || primitiveIdList.some((id) => !primitiveIds.has(id as string)) || mappingKeys.has(key)) geometryValid = false;
    else {
      mappingKeys.add(key);
...
function samplesFromManifest(manifest: unknown): PreviewSample[] {
  ...
  if (Array.isArray(record?.samples)) return record.samples as PreviewSample[];
  if (Array.isArray(record?.cells)) {
    return record.cells.map((cell, index): PreviewSample => {
      const item = rec(cell) ?? {};
      const locator = rec(item.locator);
      return {
        id: (item.id ?? `cell-${index}`) as string | undefined,
        row: item.row as string | undefined,
        size: item.size as number | undefined,
        locator: { bbox: (item.bbox ?? locator?.bbox) as number[] | undefined, region: (item.region ?? locator?.region) as string | undefined },
      };
    });
  }
  return [];
...
function validatePreviewAndAesthetic(model: LogoModel, findings: ContractFinding[]): void {
  const digest = masterSubjectDigest(model);
...
function validateVariants(model: LogoModel, findings: ContractFinding[]): void {
...
function validateEvidenceRecord(model: LogoModel, filePath: string, schema: string, requiredChecks: string[], code: string, findings: ContractFinding[]): unknown {
  ...
  const valid = isObject(record) && record.schema === schema && record.artifactId === model.artifactId && record.subjectDigest === computeLogoSubjectDigest(model)
    && requiredChecks.every((id) => checks.some((check) => rec(check)?.id === id && rec(check)?.status === "pass"));
  if (!valid) findings.push(finding(code, filePath, `${filePath} must bind the current subject and all required passing checks`));
  return record;
}

function validateRelease(model: LogoModel, findings: ContractFinding[]): void {
...
export function validateLogoModel(model: LogoModel | null | undefined, { stage = "source" }: LogoValidateOptions = {}): ContractFinding[] {
...
export function evaluateLogoWrite({ relativePath = "", toolName = "" }: LogoWriteOptions = {}): LogoWriteDecision {
