/**
 * TypeScript dependency lockfile guard (PreToolUse).
 * Failure mode: fail-closed (deny) with a blockingContract.
 */

import { basename } from "node:path";

export const LOCKFILE_NAMES = ["package-lock.json","pnpm-lock.yaml","yarn.lock","bun.lock","bun.lockb","deno.lock","npm-shrinkwrap.json"].map((n) => n.toLowerCase());
...
export function lockfileDenyMessage(targets) {
  const generatedBy = "npm/pnpm/yarn/bun/deno";
  return [
    `[TypeScript Dependency Lockfile Guard] 已拦截依赖 lock 文件修改`,
    "",
    `目标：${targets.join(", ")}`,
    "",
