...
function extractShellCommand(toolName, toolInput) {
  if (!/^(Bash|Shell|bash|shell|shell_command|exec_command|exec|local_shell)$/i.test(
    toolName
  )) {
    return null;
  }
...
function extractPatchPaths(toolInput) {
  const blob = [toolInput?.patch, toolInput?.input, toolInput?.command].filter((v) => typeof v === "string").join("\n");
  if (!blob) return [];
  const paths = [];
...
// plugins/command-safety-guards/src/lib/rule-engine.ts
import { execFileSync } from "node:child_process";
import { existsSync } from "node:fs";
import { join } from "node:path";
import { pathToFileURL } from "node:url";

// plugins/command-safety-guards/src/lib/builtin-rules.ts
import { createHash } from "node:crypto";

// plugins/command-safety-guards/src/lib/shell-parse.ts
...
// plugins/command-safety-guards/src/lib/sanitize-command.ts
