...
// plugins/command-safety-guards/src/engines/secret-read.ts
...
function escalationMessage(event, command, options = {}) {
  ...
  return count >= threshold ? `[Deny Escalation Guard] command-safety-guards has denied the same target ${count} times.
...
// plugins/command-safety-guards/src/engines/dangerous-rm.ts
...
// plugins/command-safety-guards/src/entries/hooks/cmd-safety-hook-pre-tool.ts
async function main() {
