...
5. 草稿必须包含本周成果、证据、非 AI 工作、阻碍、方法/执行不足、下周行动。运行 `weekly-work-report-prepare.mjs --week YYYY-Www --input <draft>`，展示全文；内容变化后重新 prepare。Hook 不会根据用户下一句话确认。
6. 确认正文后运行 `weekly-work-report-save.mjs --week YYYY-Www --input <draft>`，不得直接修改报告目录。事实较多时可以用自然语言请普通只读子 agent 检查遗漏，但父 agent 必须核对事实并决定是否修改。
