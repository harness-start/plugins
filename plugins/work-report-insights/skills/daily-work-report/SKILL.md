...
6. 将草稿写到报告目录之外的临时文件，运行 `daily-work-report-prepare.mjs --date YYYY-MM-DD --input <draft>`。派一个只读 critic，prompt 仅含 `WRI_REVIEW_REQUEST critic`，让它根据事实卡片写方法和缺口，不要改写成果。向员工展示完整草稿；内容有改动就重新 prepare。Hook 不会根据用户下一句话确认。
7. critic 通过后运行 `daily-work-report-save.mjs --date YYYY-MM-DD --input <draft>`。只报告工具实际返回的路径和摘要；不要直接 Write/Edit `~/.ai-experts/daily-reports/`。
