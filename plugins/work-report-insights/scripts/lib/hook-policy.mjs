...
import { extractCwd, extractFileTargets, extractShellCommand, isFileMutationTool, isShellTool } from "./hook-io.mjs";
import { parseReportArgs } from "./report-cli.mjs";
import { isProtectedReportPath, reportPath } from "./report-store.mjs";
import { sha256 } from "./report-integrity.mjs";
...
export function parseOfficialCommand(command) {
...
export async function officialScriptTrusted(official, options = {}) {
  if (!official?.script || !OFFICIAL.has(basename(official.script))) return false;
  if (/^\$\{(?:PLUGIN_ROOT|CLAUDE_PLUGIN_ROOT)\}\/scripts\/[a-z0-9-]+\.mjs$/u.test(official.script)) return true;
  const pluginRoot = resolve(options.pluginRoot ?? DEFAULT_PLUGIN_ROOT);
  const cwd = resolve(options.cwd ?? process.cwd());
  const actual = resolve(cwd, official.script);
  const expected = join(pluginRoot, "scripts", basename(official.script));
...
async function protectedCandidate(path, home) {
...
function shellTokens(command, home) {
...
async function shellTargetsReports(command, cwd, home) {
...
export async function protectionDecision(event, options = {}) {
