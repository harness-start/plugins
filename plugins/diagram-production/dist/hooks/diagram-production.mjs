...
} from "../chunks/chunk-HMY4KEAW.mjs";
import {
...
function invocation(words, cwd, workspaceRoot) {
  ...
  if (dirname(projectRoot) !== resolve2(workspaceRoot, "artifacts", "diagram") || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(basename(projectRoot))) return null;
...
var initDigest = (root) => createHash("sha256").update(`diagram-init:${resolve3(root)}`).digest("hex");
async function runPre(event) {
  const cwd = resolve3(eventCwd(event));
  for (const target of extractFileTargets(event, { tools: "any" })) {
    const result = evaluateDiagramWrite({ relativePath: relative(cwd, resolve3(cwd, target)), toolName: eventToolName(event), cwd });
