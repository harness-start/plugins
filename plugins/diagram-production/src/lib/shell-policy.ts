...
function invocation(words: string[] | null, cwd: string, workspaceRoot: string) {
  if (!words || words.length < 3) return null; const [runtime, entry, rootWord] = words;
  if (!runtime || !entry || !rootWord || !["node", basename(process.execPath), process.execPath].includes(runtime) || entry.startsWith("-")) return null;
  const script = isAbsolute(entry) ? resolve(entry) : resolve(cwd, entry); const name = basename(script);
  ...
  const exact = hasExternalInput ? words.length === 4 && isAbsolute(words[3] ?? "") : words.length === 3;
...
export function evaluateDiagramShell({ command, cwd, workspaceRoot, activeProjectCount = 0 }: { command: unknown; cwd: string; workspaceRoot: string; activeProjectCount?: number }): DiagramShellDecision {
