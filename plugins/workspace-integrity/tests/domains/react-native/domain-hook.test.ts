...
import { policy } from "../../../src/domains/react-native/entries/hooks/domain-hook.js";
...
test("react-native-engineering validators are bounded",()=>{assert.ok(policy.validators.length>0);assert.equal(policy.validators.every(v=>v.mode==="block"||v.mode==="report"),true);});
