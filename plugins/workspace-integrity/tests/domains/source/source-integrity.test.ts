...
import {
  ...
  modeFor,
  resolveConfig,
} from "../../../src/domains/source/lib/source-sanity-policy.js";
import {
  extractFileTargets,
  extractInsertedText,
  extractPatchTargets,
} from "../../../src/domains/source/entries/hooks/source-integrity.js";

const ENTRY = fileURLToPath(new URL("../../../dist/hooks/dispatcher.mjs", import.meta.url));

function runEntry(event, env = {}) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(process.execPath, [ENTRY, "codex", "PreToolUse"], {
      env: { ...process.env, ...env },
...
test("public policy and owner routes no longer own merge conflict checks", () => {
  assert.deepEqual(CHECK_NAMES, ["backupArtifact", "garbledText"]);
  for (const host of ["claude", "codex"] as const) {
    assert.deepEqual(
      Object.keys(readModuleRoutes(import.meta.url, host, "source")),
...
test("create_file and search_replace extract and deny backup artifacts", async () => {
  ...
  for (const host of ["claude", "codex"] as const) {
    const matcher = readModuleRoutes(import.meta.url, host, "source").PreToolUse[0].matcher ?? "";
    assert.match(matcher, /create_file/u, host);
    assert.match(matcher, /search_replace/u, host);
  }
