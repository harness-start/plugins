...
import {
  ...
  matchRule,
  resolveRules,
} from "../../../src/domains/source/lib/encoding-runner.js";
import { analyzeEncoding } from "../../../src/domains/source/lib/encoding-policy.js";
...
test("entry loads a project skip override before built-ins", async () => {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "source-integrity-config-")));
  try {
    ...
    const result = await runEntry(
      JSON.stringify({ cwd: root, tool_name: "Write", tool_input: { file_path: target } }),
    );
