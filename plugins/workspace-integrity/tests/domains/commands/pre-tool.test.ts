...
} from "../../../src/domains/commands/engines/dangerous-rm.js";
import { mysqlReplicationPreflightFinding } from "../../../src/domains/commands/engines/mysql-preflight.js";
import { fileSafetyReports } from "../../../src/domains/commands/engines/file-safety.js";
import { secretReadReport } from "../../../src/domains/commands/engines/secret-read.js";
import { escalationMessage, recordDeny } from "../../../src/domains/commands/lib/deny-state.js";
import {
  verificationIntegrityDenyMessage,
  verificationIntegrityFinding,
} from "../../../src/domains/commands/engines/verification-integrity.js";

const PRE = fileURLToPath(
  new URL("../../../dist/hooks/dispatcher.mjs", import.meta.url),
);
...
function runHook(event, env = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [PRE, "codex", "PreToolUse"], {
      env: { ...process.env, ...env },
...
test("plugin-local deny state escalates after three distinct turns", () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(root, ".command-safety", ".state", "denies.jsonl")), true);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
