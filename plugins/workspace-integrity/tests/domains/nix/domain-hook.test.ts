...
import { policy } from "../../../src/domains/nix/policy.js";
...
test("nix-engineering validators are bounded",()=>{assert.ok(policy.validators.length>0);assert.equal(policy.validators.every(v=>v.mode==="block"||v.mode==="report"),true);assert.equal(policy.validators.every(v=>v.enforcement==="deterministic"||v.enforcement==="advisory"),true);});
