...
import { policy } from "../../../src/domains/web/policy.js";
...
test("web-frontend-engineering validators are bounded",()=>{assert.ok(policy.validators.length>0);assert.equal(policy.validators.every(v=>v.mode==="block"||v.mode==="report"),true);assert.equal(policy.validators.every(v=>v.enforcement==="deterministic"||v.enforcement==="advisory"),true);});
