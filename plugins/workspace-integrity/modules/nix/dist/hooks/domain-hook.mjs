#!/usr/bin/env node
// harness-source-hash: sha256:ef9a57ca0a136660f86fa2836a4d8ac5a8813c3bc57a4b66aae52dc155ca25a7
import "../chunks/chunk-R7HS6YO6.mjs";

// plugins/workspace-integrity/modules/nix/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-7LB76XWW.mjs");
...
// plugins/workspace-integrity/modules/nix/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[nix-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
