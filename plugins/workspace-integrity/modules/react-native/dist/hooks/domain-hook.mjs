#!/usr/bin/env node
// harness-source-hash: sha256:a6fbb51f896f07f67a0922aedd6f7f761923040183f0c85ebc0f5704b85ee26c
import "../chunks/chunk-RNPW4GVW.mjs";

// plugins/workspace-integrity/modules/react-native/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-ZKFX457E.mjs");
...
// plugins/workspace-integrity/modules/react-native/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[react-native-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
