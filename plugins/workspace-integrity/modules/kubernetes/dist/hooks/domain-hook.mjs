#!/usr/bin/env node
// harness-source-hash: sha256:9d7aae4d924231c021f5f8f1c003e264b8c09e978c288e27702fca81e790dd48
import "../chunks/chunk-ARJXOIS2.mjs";

// plugins/workspace-integrity/modules/kubernetes/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-6O4SKTTT.mjs");
...
// plugins/workspace-integrity/modules/kubernetes/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[kubernetes-operations] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
