#!/usr/bin/env node
// harness-source-hash: sha256:1202c317a5017b6b42cc9675e4553ae9546b49babd4e46be2bc81e3c9709d29b
import "../chunks/chunk-VRCUC3HD.mjs";

// plugins/workspace-integrity/modules/web/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-CDZCRKQ3.mjs");
...
// plugins/workspace-integrity/modules/web/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[web-frontend-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
