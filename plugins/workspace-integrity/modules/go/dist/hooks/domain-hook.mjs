#!/usr/bin/env node
// harness-source-hash: sha256:2693e5b98ddefdcb42f8ca60c0cfbda5a165261da36f25ddc42be52d902db8f0
import "../chunks/chunk-LP7R6S5V.mjs";

// plugins/workspace-integrity/modules/go/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-N7AEHCVJ.mjs");
...
// plugins/workspace-integrity/modules/go/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[go-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
