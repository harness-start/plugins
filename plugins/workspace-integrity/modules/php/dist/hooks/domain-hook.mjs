#!/usr/bin/env node
// harness-source-hash: sha256:5d7d6db4fa15ca9a75926c37ee4ad5e1f8e1db015c103dae4bd74c661c513a50
import "../chunks/chunk-64UA6LLL.mjs";

// plugins/workspace-integrity/modules/php/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-XAXL3PKO.mjs");
...
// plugins/workspace-integrity/modules/php/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[php-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
