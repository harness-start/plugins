#!/usr/bin/env node
// harness-source-hash: sha256:ebecb9396f640760cc75287701cb341f245ae0b9141d7fd37d0e0412619e847c
import "../chunks/chunk-NLIAU5SG.mjs";

// plugins/workspace-integrity/modules/ios/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-5ZGOJKS6.mjs");
...
// plugins/workspace-integrity/modules/ios/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[ios-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
