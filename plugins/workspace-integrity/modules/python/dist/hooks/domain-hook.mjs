#!/usr/bin/env node
// harness-source-hash: sha256:396b43d019231d792a9da44c637bfc668472402c698ce06f331b84dc99cc33cc
import "../chunks/chunk-XBWZSHFW.mjs";

// plugins/workspace-integrity/modules/python/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-2U2BWGFO.mjs");
...
// plugins/workspace-integrity/modules/python/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[python-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
