#!/usr/bin/env node
// harness-source-hash: sha256:29857cba99898a9885ac0542a6eda1f7970eaa8db1a70d8cc1d340c546adc120
import "../chunks/chunk-3XU3EYTZ.mjs";

// plugins/workspace-integrity/modules/java/src/entries/hooks/domain-hook.ts
...
async function xmlValidation(filePath) {
  const errors = [];
  try {
    const { DOMParser } = await import("../chunks/lib-7OHYY3IG.mjs");
...
// plugins/workspace-integrity/modules/java/src/policy.ts
var policy = {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve3(process.argv[1])) runDomainEngineeringHook(policy, process.argv[2]).catch((error) => {
  process.stderr.write(`[java-engineering] hook failed open: ${error instanceof Error ? error.message : String(error)}
`);
  process.exit(0);
});
export {
