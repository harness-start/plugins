/**
 * webman-runtime-guards env detection tests (pure unit tests; the entry
...
test("signals: prompt hitting the framework keyword passes the filter", () => {
  assert.equal(shouldInject("帮我修复这个 webman 项目的登录问题"), true);
});
...
test("signals: question-only prompts without execution verbs are skipped", () => {
  assert.equal(isQuestionOnlyPrompt("什么是 webman 的路由？"), true);
  assert.equal(shouldInject("什么是 webman 的路由？"), false);
  assert.equal(shouldInject("怎么修复 webman 的迁移失败？"), true);
});

test("detect: webman composer.json yields facts", () => {
  const dir = tempDir("webman-env-");
  try {
    ...
    assert.match(facts, /\[webman/i);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("detect: non-webman composer.json yields null", () => {
  const dir = tempDir("webman-env-");
  try {
    writeFileSync(join(dir, "composer.json"), JSON.stringify({
      name: "acme/other",
