...
for probe in primary boundary composition compat; do
...
jq -e '.schema == "behavioral-regression/v10" and .surface.inputShape == "multi-component" and (.surface.callForms | length > 0) and (.surface.constraintSeam | length > 0) and (.scope.regressionPaths | length > 0)' "${contract}" >/dev/null
jq -e '([.cases[].coverage[]] | unique) as $coverage | all(["primary", "public-seam", "constraint-seam", "compatibility", "all-populated", "all-degenerate", "each-one-degenerate"][]; . as $token | $coverage | index($token))' "${contract}" >/dev/null
echo "OK auto-routed partial interaction closed with fresh receipts"
