...
状态包含 `preferredProfile`、有限的 `authorizedProfiles` 集合和 `toolFeedbackDelivered`。主 agent 与 subagent 共享父会话的 session ID。状态不保存 prompt、回复、命令或文件内容；它以 session ID 的 SHA-256 为键，原子写到当前工作目录的 `.language-output-governance/state/`。`.language-output-governance/.gitignore` 忽略该工作目录的全部内容，插件不会修改项目根目录的 `.gitignore`。状态 24 小时后过期，当前没有逐轮 profile 或撤销协议。
