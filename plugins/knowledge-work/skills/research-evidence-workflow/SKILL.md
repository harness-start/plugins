...
```bash
RPG_PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT:-}}"
test -f "$RPG_PLUGIN_ROOT/dist/cli/harness.mjs"

node "$RPG_PLUGIN_ROOT/dist/cli/harness.mjs" research run-open --cwd "$PWD"
```
...
```bash
node "$RPG_PLUGIN_ROOT/dist/cli/harness.mjs" research handoff-outbound \
  --cwd "$PWD" \
