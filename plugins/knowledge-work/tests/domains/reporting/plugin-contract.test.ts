...
test("plugin exposes one orchestrator, interview method, review skill, and no community deps", () => {
  const skillNames = readdirSync(join(ROOT, "skills"), { withFileTypes: true }).filter((entry) => entry.isDirectory() && entry.name.startsWith("work-report-")).map((entry) => entry.name).sort();
