...
test("Codex bundled MCP config uses a supported direct server map", async () => {
  ...
  assert.deepEqual(config.research_provenance.args, ["./dist/mcp/research-provenance-server.mjs"]);
...
test("Claude MCP config inherits the platform plugin-data directory", async () => {
  ...
  assert.deepEqual(server.args, ["${CLAUDE_PLUGIN_ROOT}/dist/mcp/research-provenance-server.mjs"]);
