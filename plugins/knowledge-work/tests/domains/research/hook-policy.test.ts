...
import { evaluateStop } from "../../../src/domains/research/entries/hooks/evidence-based-research.js";
import { appendStateEvent, readState } from "../../../src/domains/research/lib/state-store.js";
import { defaultWorkflow, ensureRunSkeleton, readWorkflowFile, workflowPath, writeWorkflow } from "../../../src/domains/research/lib/workflow-fs.js";
...
function runHook(mode, event, dataRoot) {
  const script = fileURLToPath(new URL("../../../dist/hooks/dispatcher.mjs", import.meta.url));
  const pluginRoot = resolve(fileURLToPath(new URL("../../../", import.meta.url)));
  return spawnSync(process.execPath, [script, "codex", ({ "session-start": "SessionStart", pre: "PreToolUse", post: "PostToolUse", failure: "PostToolUseFailure", stop: "Stop", session: "SessionStart", prompt: "UserPromptSubmit", "user-prompt": "UserPromptSubmit", subagent: "SubagentStart", "subagent-stop": "SubagentStop" } as Record<string, string>)[mode] ?? mode], {
...
test("sealed evidence remains reusable across outbound and implementation mutations", async () => {
  ...
  const lifecycle = runHook("post", {
    ...event,
    tool_name: "exec_command",
    tool_input: { cmd: `node "${pluginRoot}/dist/cli/harness.mjs" research handoff-outbound --cwd "${workspace}" --handoff-file /tmp/h --prompt-file /tmp/p` },
  }, dataRoot);
  ...
  const chained = runHook("post", {
    ...event,
    tool_name: "exec_command",
    tool_input: { cmd: `node "${pluginRoot}/dist/cli/harness.mjs" research handoff-outbound --cwd "${workspace}" --handoff-file /tmp/h --prompt-file /tmp/p; echo changed > notes.md` },
  }, dataRoot);
...
test("research_begin preflight uses state and creates only the research-local gitignore", async () => {
  ...
  const result = spawnSync(process.execPath, [script, "codex", "PreToolUse"], {
...
test("Claude plugin-prefixed MCP tool names are recognized and receipts persist", async () => {
  ...
  const result = runHook("post", {
    ...event,
    tool_name: "mcp__plugin_evidence-based-research_research_provenance__research_seal",
