...
Run from the repository root with `scripts/acceptance/run.sh --plugin evidence-based-research`. All cases require fresh Claude Code and Codex sessions with the plugin MCP server and hooks loaded.
...
- `04-direct-firecrawl-denied`: open run + PreToolUse proves external Firecrawl discovery stays disabled, then the run seals and completes.
