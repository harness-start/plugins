...
作者刻意用对比结构承载全文论点时，仍按上述保留条件逐项判断。不要为单篇文本创造新的通用豁免。
...
**检测**：`进行了 X` / `实现了 X 的 Y` / `完成了对 X 的 Y` / `起到了 X 的作用` / `具有 X 的意义` / `开展 X 工作`。英文对应 `perform an analysis of`、`conduct a review of`、`achieve an improvement in`。自动统计只能作为定位信号，不能代替语境判断。
...
**检测**：**只在 `public-writing` 的叙事、观点、随笔类文本上判，`docs` / `status` / `code-context` 不判。** 不设全局密度阈值：技术文本常靠 `如果…否则`、`因为…所以` 承担条件和因果，按密度一刀切会优先误伤它们。
...
**检测**：短距离内（约 800 字窗口）命中 3 套以上借喻场。统计时排除字面用法（`搜索引擎`、`代码仓库`、`商品库存` 不计）。
