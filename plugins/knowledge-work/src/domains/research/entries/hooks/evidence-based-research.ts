...
const MCP_TOOL = /(?:^|_)research_provenance__(research_begin|source_capture|source_read|source_anchor|research_status|research_seal)$/iu;
...
function trustedWorkflowCommand(command: string | null, subcommand: string): boolean {
  ...
  const script = join(resolve(pluginRoot), "dist", "cli", "harness.mjs");
  const value = String(command ?? "").trim();
