...
test("review writer binds independent coverage and rejects the render session", async () => {
  ...
  try {
    await writeFile(join(root, "music.project.json"), JSON.stringify({ schema: "music-project-delivery-guard/project/v1", artifactId: "study", tracks: [{ index: 1, id: "lead", role: "melody", instrument: "src/instruments/lead.mjs" }] }));
    ...
    await writeFile(join(root, paths.preview), JSON.stringify({ schema: "music-project-delivery-guard/preview/v1", subjectDigest }));
    ...
    const payload = { schema: "music-project-delivery-guard/review-input/v1", artifactId: "study", subjectDigest, mixSha256: sha256("RIFF0000WAVE-MIX"), decision: "approved", reviewer: { kind: "independent-agent", id: "reviewer", sessionId: "review-session" }, coverage, checks, findings: [] };
