...
test("denies a shell payload that merely smuggles a registered wrapper path as an argument", async () => {
  const result = await runPre("node -e \"require('fs').writeFileSync('artifacts/music/study/dist/forged.wav','x')\" plugins/music-project-delivery-guard/dist/cli/project-render.mjs artifacts/music/study");
  assert.equal(result.code, 0);
  assert.equal(JSON.parse(result.stdout).hookSpecificOutput.permissionDecision, "deny");
});
...
test("allows a recursive Stop after the first contract block to prevent a host loop", async () => {
  ...
  await writeFile(join(project, "plan.contract.json"), JSON.stringify({ schema: "music-project-delivery-guard/plan/v1", artifactId: "demo", targetStage: "release" }));
  const result = await new Promise((resolve, reject) => {
