...
`music-project-delivery-guard` 为 Codex 和 Claude 提供一条可审计的器乐生产链：

```text
brief → direction → composition → arrangement → optimize → render → preview → independent review → release
```
...
外部顾问固定在 `skill-deps.json`：英文 `music-composition`、`workflow-audio-production`、`workflow-analysis-quality`，以及中文生态 `miaoxiang-music`。它们只有 adviser/reference 权限，禁止执行外部脚本、生成器、网络发布或直接写项目。`music-composition` 的方法文档采用 CC-BY-4.0，本插件在依赖清单中保留来源和许可归属。

## 快速开始

```bash
export PLUGIN_ROOT="$PWD/plugins/music-project-delivery-guard"
...
独立 reviewer 必须覆盖当前 brief、score、render receipt、preview、mix 和全部 stems，并检查 brief 对齐、旋律和声、节奏律动、曲式编排、音色配器、平衡/空间/动态以及技术完整性。finding 必须绑定精确路径和 SHA-256；批准时 blocker/major 必须有复核证据。

reviewer 在项目外准备 `music-project-delivery-guard/review-input/v1` JSON，再通过：
...
Hooks 对 mutating CLI 签发短时、一次性、绑定 argv/session/subject digest 的 capability。命令拼接、重定向、wrapper 伪装、直接写 `plan.contract.json`、`build/`、`proofs/`、`evidence/`、`review.music.json`、`dist/`、manifest 和 receipt 都会被拒绝。Post/Failure/Stop hooks 会报告未关闭 journal、陈旧 evidence 或未完成 release。
...
```bash
npx tsx --test plugins/music-project-delivery-guard/tests/*.test.ts
npm run check:dist
./scripts/acceptance/run.sh --plugin music-project-delivery-guard
```
