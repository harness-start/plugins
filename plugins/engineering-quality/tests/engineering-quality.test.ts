...
import {
  ...
  resolveConfig,
  writeState,
} from "../src/lib/code-quality-core.js";
import { extractFileTargets } from "../src/entries/hooks/static-code-check.js";

const POST_ENTRY = fileURLToPath(new URL("../dist/hooks/engineering-quality-post.mjs", import.meta.url));
const STOP_ENTRY = fileURLToPath(new URL("../dist/hooks/engineering-quality-stop.mjs", import.meta.url));
...
test("state writes a root gitignore and uses the visible state directory", () => {
  ...
  try {
    ...
    assert.equal(readFileSync(join(root, ".engineering-quality", ".gitignore"), "utf8"), "*\n");
    assert.equal(existsSync(join(root, ".engineering-quality", "state")), true);
    assert.equal(readdirSync(join(root, ".engineering-quality", "state")).length, 1);
    assert.equal(existsSync(join(root, ".engineering-quality", ".state")), false);
  } finally {
...
test("state upgrades a stale subdirectory gitignore to ignore the whole workdir", () => {
  ...
  try {
    mkdirSync(join(root, ".engineering-quality"), { recursive: true });
    writeFileSync(join(root, ".engineering-quality", ".gitignore"), "state/\n", "utf8");
...
test("JavaScript syntax errors block after the write", async () => {
  ...
  try {
    ...
    writeFileSync(join(root, ".engineering-quality.mjs"), "export default { checks: { eslint: 'off' } };\n", "utf8");
...
test("non-Git directories use built-ins but do not load a cwd config", async () => {
  ...
  try {
    ...
    const target = join(root, "src", "app.js");
    writeFileSync(target, "const value = ;\n", "utf8");
    writeFileSync(join(root, ".engineering-quality.mjs"), [
      "export default {",
...
test("PHP files persist for Stop and PHPStan report remains non-blocking", async () => {
  ...
  try {
    ...
    writeFileSync(join(root, ".engineering-quality.mjs"), "export default { checks: { phpSyntax: 'off' } };\n", "utf8");
...
test("PHPStan can be configured as a Stop blocker", async () => {
  ...
  try {
    ...
    const target = join(root, "src", "App.php");
    writeFileSync(target, "<?php\n", "utf8");
    writeFileSync(join(root, ".engineering-quality.mjs"), "export default { checks: { phpSyntax: 'off', phpstan: 'block' } };\n", "utf8");
    writeFileSync(join(root, "phpstan.neon"), "parameters:\n  level: 5\n", "utf8");
