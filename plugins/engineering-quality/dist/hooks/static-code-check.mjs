...
import {
  ...
  resolveRepoRoot,
  runCommand
} from "../chunks/chunk-AUNJKAID.mjs";
import {
  extractFileTargets,
  extractShellCommand,
  isFileMutationTool,
  isShellTool
} from "../chunks/chunk-4J7VXMOW.mjs";
import {
  eventCwd,
  eventToolName,
  isRecord,
  readStdinJson
} from "../chunks/chunk-J4Y3Q6SV.mjs";

// plugins/engineering-quality/src/entries/hooks/static-code-check.ts
...
function extractFileTargets2(event) {
  if (isShellTool(eventToolName(event))) {
    ...
    return [...new Set(
      extractShellWriteTargets(extractShellCommand(event) ?? "").filter(Boolean).map((path) => isAbsolute(path) ? resolve(path) : resolve(cwd, path.replace(/^\.\//u, "")))
    )];
  }
