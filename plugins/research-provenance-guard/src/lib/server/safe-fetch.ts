...
async function requestOnce(url: URL, timeoutMs: number, maxBytes: number): Promise<HttpFetchResponse> {
...
export function pinnedLookup(addresses: LookupAddress[]): LookupFunction {
  ...
  return function lookupPinned(_hostname: string, options: LookupOptions, callback: (err: NodeJS.ErrnoException | null, address: string | LookupAddress[], family?: number) => void): void {
...
export async function safeFetchText(value: string, { timeoutMs = 15_000, maxBytes = MAX_BYTES, maxRedirects = 5 }: SafeFetchOptions = {}): Promise<FetchedText> {
