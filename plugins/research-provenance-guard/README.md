...
捕获内容和 hook 事件写在当前工作目录的 `.research/state/`。`.research/.gitignore` 只忽略 `state/`，插件不会修改项目根目录的 `.gitignore`。`.mcp.json` 不得用 `CLAUDE_PLUGIN_DATA` 占位符覆盖平台环境；工作区路径由 MCP workspace root 解析。
