...
import {
  evaluatePrintWrite,
  validatePrintModel
} from "../chunks/chunk-SZ4PLWWG.mjs";
...
var cwdOf = (event) => resolve(event?.cwd ?? event?.working_directory ?? event?.workingDirectory ?? process.cwd());
...
async function findingsFor(cwd) {
  ...
  for (const root of await discover(cwd)) {
    ...
    try {
      project = JSON.parse(files["print.project.json"]);
    } catch {
    }
