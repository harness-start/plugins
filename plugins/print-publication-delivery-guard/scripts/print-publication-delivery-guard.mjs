...
import { evaluatePrintWrite, validatePrintModel } from "./lib/contract.mjs";
...
function deny(reason) {
  return { hookSpecificOutput: { hookEventName: "PreToolUse", permissionDecision: "deny", permissionDecisionReason: `[Print Project Delivery Guard] ${reason}` } };
}
...
async function discover(cwd) {
  const root = join(cwd, "artifacts", "print");
  try {
...
async function findingsFor(cwd) {
  ...
  for (const root of await discover(cwd)) {
    ...
    try { project = JSON.parse(files["print.project.json"]); } catch {}
    const model = { artifactId: basename(root), files, digests, plan, project };
    for (const item of validatePrintModel(model, { stage: plan?.targetStage ?? "source" })) findings.push({ artifactId: model.artifactId, ...item });
  }
  return findings;
}

function format(findings) {
  return ["[Print Project Delivery Guard] Project contract violations", ...findings.slice(0, 50).map((item) => `- ${item.artifactId}/${item.path} [${item.code}] ${item.message}`), "recovery: Fix the named variant, layer, proof, or output and rerun the registered print tool."].join("\n");
}
...
async function main() {
  ...
  if (mode === "pre") {
    for (const target of targetsOf(event)) {
      const result = evaluatePrintWrite({ relativePath: relative(cwd, resolve(cwd, target)), toolName: nameOf(event) });
    ...
    const compoundShell = /(?:&&|\|\||[;&|><`\n]|\$\()/u.test(command);
    const approvedWrapper = /print-publication-delivery-guard[\\/]scripts[\\/]tools[\\/](?:project-lint|project-release)\.mjs\b/u.test(command) && !compoundShell;
    if (mutates && !approvedWrapper) process.stdout.write(`${JSON.stringify(deny("UNKNOWN_MUTATION_SHELL: print mutations require a registered wrapper"))}\n`);
    return;
  }
  const findings = await findingsFor(cwd);
  if (mode === "session") {
    if ((await discover(cwd)).length > 0) process.stdout.write(`${JSON.stringify(context("SessionStart", "[Print Project Delivery Guard] active; generated outputs require registered writers."))}\n`);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) main().catch((error) => { process.stderr.write(`[Print Project Delivery Guard] ${error.message}\n`); process.exitCode = 2; });
