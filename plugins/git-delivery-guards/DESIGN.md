...
最终文件必须是受支持的文本扩展名、普通文件且不超过 2 MiB。只匹配行首标准七字符标记，且文件必须至少包含一个高特异性的 `<<<<<<<` 或 `>>>>>>>` 边界；单独出现的 `=======` 可能是 RST 表格边框，不视为冲突。最多报告 10 个位置。`block` 使 PostToolUse 失败，但不尝试回滚已经完成的写入；`report` 只注入上下文。
