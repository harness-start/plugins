...
import { RENDER_EVIDENCE_SCHEMA, computePptxSubjectDigest, inspectPptxPackage, loadPptxProject, validatePptxModel } from "../../lib/contract.js";
import { consumeWriterCapability, processWriterArgv } from "../../lib/capability.js";
import { pdfPageCount, renderOfficePages, toolVersion } from "../../lib/office.js";
import { assertPptxProjectRoot, atomicWriteJson, sessionMetadata, withWriterJournal } from "../../lib/writer.js";

function runProjectRenderer(root: string, output: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.on("error", (error) => { clearTimeout(timer); reject(new Error(`RENDER_SCRIPT_UNAVAILABLE:${error.message}`)); });
    child.on("close", (code) => { clearTimeout(timer); if (code === 0) resolve(); else reject(new Error(`RENDER_SCRIPT_FAILED:${stderr.trim()}`)); });
  });
}
...
async function main() {
  ...
  const manifest = JSON.parse(String(model.files?.["src/slides/manifest.json"])) as { slides: Array<{ source: string }> };
  await withWriterJournal(root, "pptx-render", async () => {
    ...
    try {
      ...
      await atomicWriteJson(root, "evidence.render.json", {
        schema: RENDER_EVIDENCE_SCHEMA, plugin: "pptx-project-delivery-guard", artifactId: model.artifactId,
        subjectDigest: computePptxSubjectDigest(model), output: { pptxSha256: model.digests?.[`dist/${model.artifactId}.pptx`], pdfSha256: model.digests?.[`dist/${model.artifactId}.pdf`], pages: rendered.pages.map((page, index) => ({ index: index + 1, path: `dist/pages/${String(index + 1).padStart(3, "0")}.png`, sha256: model.digests?.[`dist/pages/${String(index + 1).padStart(3, "0")}.png`] })) },
        pageCount, tools: { soffice: await toolVersion("soffice"), pdftoppm: await toolVersion("pdftoppm", ["-v"]).catch(() => "pdftoppm") }, ...sessionMetadata("pptx-render", grant),
      });
...
main().catch((error: unknown) => { process.stderr.write(`[pptx-project-render] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
