...
export const PLAN_SCHEMA = "pptx-project-delivery-guard/plan/v2";
export const STORYBOARD_SCHEMA = "pptx-project-delivery-guard/storyboard/v2";
export const SKILL_COMPOSITION_SCHEMA = "pptx-project-delivery-guard/skill-composition/v1";
export const DESIGN_SYSTEM_SCHEMA = "pptx-project-delivery-guard/design-system/v1";
export const PROJECT_SCHEMA = "pptx-project-delivery-guard/project/v2";
export const SLIDE_MANIFEST_SCHEMA = "pptx-project-delivery-guard/slide-manifest/v2";
export const RENDER_EVIDENCE_SCHEMA = "pptx-project-delivery-guard/render-evidence/v1";
export const STRUCTURE_EVIDENCE_SCHEMA = "pptx-project-delivery-guard/structure-evidence/v2";
export const DESIGN_EVIDENCE_SCHEMA = "pptx-project-delivery-guard/design-evidence/v1";
export const ACCESSIBILITY_EVIDENCE_SCHEMA = "pptx-project-delivery-guard/accessibility-evidence/v2";
export const REVIEW_SCHEMA = "pptx-project-delivery-guard/review/v2";
export const RELEASE_MANIFEST_SCHEMA = "pptx-project-delivery-guard/release-manifest/v2";
export const RECEIPT_SCHEMA = "pptx-project-delivery-guard/receipt/v2";

export type PptxStage = "source" | "design" | "render" | "probe" | "review" | "release";
...
export type PptxWriteOptions = { relativePath?: string; toolName?: string; writer?: string; cwd?: string };
export type PptxWriteDecision = { decision: "allow" } | { decision: "deny"; code: string; message: string };
export type PptxLoadLimits = { maxFiles?: number; maxBytesPerFile?: number; maxTextBytes?: number };
...
const STAGES = new Set<PptxStage>(["source", "design", "render", "probe", "review", "release"]);
const STAGE_RANK: Record<PptxStage, number> = { source: 0, design: 1, render: 2, probe: 3, review: 4, release: 5 };
...
const TEXT_EXTENSIONS = new Set([".cjs", ".js", ".json", ".jsx", ".md", ".mjs", ".toml", ".ts", ".tsx", ".txt", ".yaml", ".yml"]);
...
const bytesOf = (value: FileContent | undefined): Buffer => Buffer.isBuffer(value) ? value : Buffer.from(typeof value === "string" ? value : "");
const finding = (code: string, path: string, message: string): ContractFinding => ({ code, path, message });
const stageAtLeast = (stage: PptxStage, expected: PptxStage) => STAGE_RANK[stage] >= STAGE_RANK[expected];
...
function parseJson(files: FileMap, filePath: string, findings?: ContractFinding[]): unknown {
  ...
  catch { findings?.push(finding("JSON_INVALID", filePath, `${filePath} must contain valid JSON`)); return null; }
}

function schemaRecord(files: FileMap, filePath: string, schema: string, code: string, findings: ContractFinding[]) {
...
function releaseOutputPaths(model: PptxModel | null | undefined): string[] {
  return Object.keys(model?.files ?? {})
    .filter((filePath) => GENERATED_PATH.test(filePath) || (filePath.startsWith("src/slides/") && filePath.endsWith(".png")))
...
export function validatePptxReceipt(model: PptxModel | null | undefined, stage = "release"): boolean {
  ...
  let receipt: unknown;
  try { receipt = JSON.parse(text) as unknown; } catch { return false; }
...
function validateRequiredSource(files: FileMap, findings: ContractFinding[]) {
  for (const filePath of [
    ...
    "plan.skill-composition.json", "design.system.json", "pptx.project.json", "src/deck.ts", "src/theme.ts", "src/slides/manifest.json",
...
function validateDesignSystem(record: JsonRecord | undefined, findings: ContractFinding[]) {
...
function validateSourceSchemas(model: PptxModel, files: FileMap, findings: ContractFinding[]) {
  const plan = schemaRecord(files, "plan.contract.json", PLAN_SCHEMA, "PLAN_INVALID", findings);
  if (plan && (plan.artifactId !== model.artifactId || !STAGES.has(plan.targetStage as PptxStage) || typeof plan.audience !== "string" || typeof plan.objective !== "string" || typeof plan.language !== "string")) findings.push(finding("PLAN_INVALID", "plan.contract.json", "plan must bind artifactId, targetStage, audience, objective, and language"));
...
function validateSlideSource(files: FileMap, entry: unknown, findings: ContractFinding[]) {
  ...
  if (!sourceMatch) { findings.push(finding("SLIDE_NAME_INVALID", sourcePath, "slide source must use NNN-slug.ts")); return; }
  ...
  if (typeof source !== "string") { findings.push(finding("SLIDE_SOURCE_MISSING", sourcePath, "manifest slide source is missing")); return; }
...
function validateManifest(files: FileMap, storyboardSlides: unknown[], findings: ContractFinding[]) {
...
function xmlRelationships(xml: string) {
  const relationships: Array<{ id: string; target: string; type: string; external: boolean }> = [];
  const document = new DOMParser({
    onError: (level, message) => { if (level === "fatalError" || level === "error") throw new Error(`PPTX_XML_INVALID:${message}`); },
...
export type PptxPackageInspection = { slideCount: number; requiredParts: string[]; externalRelationships: string[]; unresolvedRelationships: string[] };

export function inspectPptxPackage(bytes: Buffer): PptxPackageInspection {
  if (bytes.length < 4 || bytes[0] !== 0x50 || bytes[1] !== 0x4b) throw new Error("PPTX_ZIP_SIGNATURE_INVALID");
  let total = 0;
  ...
  for (const name of [...names].filter((entry) => entry.endsWith(".rels"))) {
    ...
    for (const relationship of xmlRelationships(decode(name))) {
      if (relationship.external) { externalRelationships.push(`${name}:${relationship.target}`); continue; }
...
function validateRendered(model: PptxModel, slides: unknown[], findings: ContractFinding[]) {
  ...
  } catch (error) { findings.push(finding("PPTX_INVALID", pptxPath, error instanceof Error ? error.message : String(error))); }
  ...
  for (const pagePath of pagePaths) try { inspectPng(bytesOf(files[pagePath])); } catch { findings.push(finding("PNG_INVALID", pagePath, "rendered page must be a valid PNG")); }
  for (const entry of slides) {
    ...
    else try { inspectPng(bytesOf(files[preview])); } catch { findings.push(finding("PNG_INVALID", preview, "slide preview must be a valid PNG")); }
  }
...
function validateEvidence(model: PptxModel, slides: unknown[], findings: ContractFinding[]) {
  const files = model.files ?? {};
  const pptxPath = `dist/${model.artifactId}.pptx`;
...
function validateReview(model: PptxModel, slides: unknown[], findings: ContractFinding[]) {
  const files = model.files ?? {};
...
export function validatePptxModel(model: PptxModel | null | undefined, { stage = "source" }: PptxValidateOptions = {}): ContractFinding[] {
  if (typeof stage !== "string" || !STAGES.has(stage as PptxStage)) return [finding("STAGE_INVALID", "plan.contract.json", `unsupported PPTX stage: ${String(stage)}`)];
...
export function evaluatePptxWrite({ relativePath = "", toolName = "", writer = "", cwd = "" }: PptxWriteOptions = {}): PptxWriteDecision {
  ...
  if (generated && !/^pptx-(?:render|probe|review|release)$/u.test(writer)) return { decision: "deny", code: "PROTECTED_WRITER_REQUIRED", message: `${inside} must be written by a registered pptx-project-delivery-guard writer, not ${toolName || "an unregistered tool"}` };
...
function isTextPath(filePath: string) { return TEXT_BASENAMES.has(basename(filePath)) || TEXT_EXTENSIONS.has(extname(filePath).toLowerCase()); }

async function hashFile(filePath: string, maxBytes: number, collectBytes: boolean) {
  ...
  const chunks: Buffer[] = [];
  let bytes = 0;
  for await (const chunk of createReadStream(filePath)) { bytes += chunk.byteLength; hash.update(chunk); if (collectBytes) chunks.push(chunk); }
...
async function collect(root: string, directory: string, state: { files: FileMap; digests: DigestMap; sizes: Record<string, number>; count: number }, limits: { maxFiles: number; maxBytesPerFile: number; maxTextBytes: number }) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    const absolute = join(directory, entry.name);
    const relativePath = relative(root, absolute).replaceAll("\\", "/");
    if (entry.isSymbolicLink()) throw new Error(`SYMLINK_REJECTED:${relativePath}`);
    if (entry.isDirectory()) { if (!SKIPPED_DIRECTORIES.has(entry.name)) await collect(root, absolute, state, limits); continue; }
    ...
    else if (text) { try { state.files[relativePath] = UTF8.decode(result.content); } catch { throw new Error(`PROJECT_TEXT_ENCODING_INVALID:${relativePath}`); } }
...
export async function loadPptxProject(projectRoot: string, limits: PptxLoadLimits = {}): Promise<PptxModel> {
  ...
  const state = { files: {} as FileMap, digests: {} as DigestMap, sizes: {} as Record<string, number>, count: 0 };
...
export async function findPptxProjects(cwd: string, { maxProjects = 32 }: { maxProjects?: number } = {}) {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const carrierRoot = join(workspaceRoot, "artifacts", "pptx");
  let entries;
  try { entries = await readdir(carrierRoot, { withFileTypes: true }); }
  ...
  for (const entry of entries) {
    ...
    try { if ((await lstat(join(root, "plan.contract.json"))).isFile()) roots.push(root); }
