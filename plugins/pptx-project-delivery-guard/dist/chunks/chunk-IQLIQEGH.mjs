...
} from "./chunk-7ARR47VX.mjs";

// plugins/pptx-project-delivery-guard/src/lib/writer.ts
...
async function withWriterJournal(root, capability, callback, grant = {}) {
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schemaVersion: 2, plugin: "pptx-project-delivery-guard", operation: capability, artifactId: basename(root), ...sessionMetadata(capability, grant) })}
`);
    await handle.sync();
  } finally {
    await handle.close();
