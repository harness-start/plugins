...
test("owner routes the private CI guard and its delivery skill stays self-contained", () => {
  assertModuleRoutedOnBothHosts(import.meta.url, "ci");
  assert.equal(existsSync(join(ROOT, "skill-deps.json")), false);
  assert.equal(existsSync(join(ROOT, "src", "domains", "ci", "entries", "hooks", "ci-gated-delivery.ts")), true);
...
test("plugin ships dual-host acceptance for remote-state boundaries and reviewer handoff", () => {
  for (const name of [
    ...
    const root = join(ROOT, "acceptance", "cases", `ci-${name}`);
