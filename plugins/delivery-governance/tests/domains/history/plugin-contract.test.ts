...
test("owner routes the private history guard and its migration skill stays bundled", () => {
  ...
  assert.equal(existsSync(join(ROOT, "src", "domains", "history", "entries", "hooks", "repository-history-migration.ts")), true);

  const skillNames = readdirSync(join(ROOT, "skills"), { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .filter((name) => name === "repository-history-migration");
...
test("plugin publishes readable scripts and appears once in both marketplaces", () => {
  for (const file of [
    "src/domains/history/lib/history-migration.ts",
    "dist/cli/harness.mjs",
  ]) {
...
test("plugin ships dual-host acceptance for refusal, success, and stale seals", () => {
  for (const name of [
    ...
    const root = join(ROOT, "acceptance", "cases", `history-${name}`);
