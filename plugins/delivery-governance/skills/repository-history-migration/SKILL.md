...
The plugin PreToolUse Hook refuses agent-issued `git filter-repo`, `git reset --hard`, and force-push against the source. It cannot prove Git history or filesystem atomicity. The deterministic owner CLI remains the mutation authority; execute only through `dist/cli/harness.mjs migration execute`.
...
AI_EXPERTS_TRIGGER_FROM="repository-history-migration:preflight" \
node "${PLUGIN_ROOT}/dist/cli/harness.mjs migration preflight" \
