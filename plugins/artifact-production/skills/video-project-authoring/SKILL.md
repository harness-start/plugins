...
8. Run `node ${PLUGIN_ROOT}/dist/cli/harness.mjs video probe`. Resolve media, timing, audio, caption, motion, shot, and conditional reference findings before review. Pass every reported near-black candidate to the independent reviewer; do not treat a candidate as an automatic failure or silently waive it.
