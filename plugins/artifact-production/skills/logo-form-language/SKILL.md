...
2. **Define the Design Brief**: Synthesize the gathered information into a structured design brief with a communication core, mandatory constraints, and prohibited drift. Choose the primary carrier and explicitly decide whether an initialism helps the audience recover the brand meaning. Do not assume that every project needs an icon, lettermark, or acronym.

3. **Generate Concept Directions**: Assess symbolic, typographic, monogram, negative-space, geometric, and narrative routes. Reject unsuitable routes with a reason, explore at least three distinct black-and-white mechanisms, and state each concept's core contribution, memory cue, retell line, invariant, and risk. Describe form logic before color or presentation styling.

4. **Refine the Selected Concept**: Once the user selects a direction, iterate on it. Adjust proportions, letter-spacing, icon weight, and color balance. For a lettermark or monogram, document the target anatomy, transformation operation, semantic cue, stroke control, counter control, axis control, negative-space control, and legibility risk. Test the design at multiple sizes — a 16x16 favicon, a 48x48 app icon, a 200px web header, and a large-format print version — to confirm legibility and visual impact at every scale.
