...
The plugin stores only the state required by each production contract. It does not rely on development-workspace caches or globally installed Skills after publication. Generated `dist/` runtimes for the owner and each private module are committed and hash-bound to their TypeScript sources.
...
node --import tsx --test \
  ...
  plugins/artifact-production/modules/*/tests/*.test.ts
npm run check:dist
```
