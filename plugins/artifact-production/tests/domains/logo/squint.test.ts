...
import { decodePngToRgba } from "../../../src/domains/logo/lib/png-decode.js";
...
import { renderPreviewStrip } from "../../../src/domains/logo/lib/preview-strip.js";
import { analyzeSquintCell, buildSquintEvidence } from "../../../src/domains/logo/lib/squint.js";

const PREVIEW_ENTRY = fileURLToPath(new URL("../../../dist/cli/harness.mjs", import.meta.url));
...
test("project preview runs with an isolated HOME and no external Skill tool", realFfmpegTest, async () => {
  ...
  try {
    ...
    await issueWriterCapability({ root: project, capability: "logo-preview", argv: [PREVIEW_ENTRY, "logo", "preview", project], subjectDigest: computeLogoSubjectDigest(await loadLogoProject(project)), sessionId: "preview-session" });
    const result = spawnSync(process.execPath, [PREVIEW_ENTRY, "logo", "preview", project], {
      encoding: "utf8",
      env: {
        ...process.env,
        HOME: isolatedHome,
...
test("project preview rejects the removed review-writing option", async () => {
  ...
  try {
    const result = spawnSync(process.execPath, [PREVIEW_ENTRY, "logo", "preview", sandbox, "--write-review"], { encoding: "utf8" });
