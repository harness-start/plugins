...
import { issueWriterCapability } from "../../../src/domains/presentation/lib/capability.js";
import { computePptxSubjectDigest, loadPptxProject } from "../../../src/domains/presentation/lib/contract.js";
...
function run(root: string) {
  return new Promise<{ code: number | null; stdout: string; stderr: string }>((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "presentation", "release", root], {
...
test("release wrapper consumes a one-time grant and writes a source-and-output-bound receipt", async () => {
  ...
  const root = join(realpathSync(workspace), "artifacts", "pptx", "deck");
  try {
    execFileSync("git", ["init", "-q"], { cwd: workspace });
    mkdirSync(root, { recursive: true });
    writeModel(root, releaseModel());
    const model = await loadPptxProject(root);
    await issueWriterCapability({ root, capability: "pptx-release", argv: [ENTRY, "presentation", "release", root], subjectDigest: computePptxSubjectDigest(model), sessionId: "release-session", triggerFrom: "test:release" });
