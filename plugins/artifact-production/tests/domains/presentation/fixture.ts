...
export function sourceModel(artifactId = "deck"): PptxModel {
  const slide = "export function renderSlide(slide, ctx) { slide.addText(ctx.copy.displayTitle, { objectName: `pptx:title:${ctx.entry.id}:display` }); }\n";
  ...
  const files: Record<string, FileContent> = {
    ...
    "plan.storyboard.json": JSON.stringify({ schema: STORYBOARD_SCHEMA, slides: [{ index: 1, id: "opening", displayTitle: "Opening", headline: { mode: "label" }, role: "opening", visual: { type: "hero", logic: "statement", variant: "statement" }, assertion: "Approve the coordinated operating model.", narrativeJob: "state-decision", transition: "establish the decision before evidence", coreContribution: "States the exact retell target." }] }),
    ...
    "design.system.json": JSON.stringify({ schema: DESIGN_SYSTEM_SCHEMA, colors: { roles: { canvas: "FFFFFF", surface: "F4F6F8", textPrimary: "111827", textSecondary: "374151", accent: "1D4ED8", success: "15803D", warning: "A16207", error: "B91C1C" } }, typography: { roles: Object.fromEntries(["display", "title", "section", "body", "list", "caption", "numeric"].map((role) => [role, { fontFamily: "Noto Sans CJK SC", fontSizePt: role === "body" ? 22 : role === "list" ? 20 : 28, lineSpacingMultiple: role === "body" ? 1.35 : role === "list" ? 1.3 : 1.15, paragraphSpaceAfterPt: role === "body" || role === "list" ? 6 : 0, charSpacingPt: 0, maxLines: role === "body" ? 6 : role === "list" ? 8 : role === "display" || role === "title" ? 1 : 2, scriptPolicy: "mixed", horizontalAlign: role === "numeric" ? "center" : "left", verticalAlign: role === "numeric" ? "middle" : "top", marginPt: 0 }])) }, spacing: { pageMarginIn: 0.5, baseUnitIn: 0.125, blockGapIn: 0.3 }, antiPatterns: ["brief-leakage", "formulaic-headline-chain", "repeated-layout", "decorative-connector"] }),
    ...
    "src/text-layout.ts": "export const addTextBlock = {};\n",
    "src/theme.ts": "export const theme = {};\n",
    "src/slides/manifest.json": JSON.stringify({ schema: SLIDE_MANIFEST_SCHEMA, slides: [{ index: 1, id: "opening", displayTitle: "Opening", role: "opening", visual: { type: "hero", logic: "statement", variant: "statement" }, source: "001-opening.ts", accessibility: { title: "Opening", readingOrder: ["title"], colorEncoding: ["color", "label"] } }] }),
...
export function releaseModel(artifactId = "deck") {
  ...
  files["evidence.design.json"] = JSON.stringify({ schema: DESIGN_EVIDENCE_SCHEMA, ...base, designSystemSha256: model.digests?.["design.system.json"], verdict: "pass", checks: [
    ...
    ...["display", "title", "section", "body", "list", "caption", "numeric"].map((role) => ({ criterion: `typography:${role}`, source: "design-system-measurement", status: "pass" })),
  ], layoutRhythm: { pages: [{ index: 1, fingerprint: [], bodyObjectCount: 0 }], similarGroups: [], similarityThreshold: 0.85 }, textRhythm: { pages: [{ index: 1, governedTextObjects: 1, autofitObjects: 0 }] }, headlineSignals: [], compositionSignals: [{ page: 1, logic: "statement", signals: [] }], deckSignals: [], textFitSignals: [] });
  ...
  files["review.pptx.json"] = JSON.stringify({ schema: REVIEW_SCHEMA, ...base, verdict: "pass", reviewer: { kind: "independent-agent", id: "reviewer-1", sessionId: "review-session" }, pages: [{ index: 1, sha256: model.digests?.[pagePath], verdict: "pass", audits: { headlineVoice: { status: "pass", classification: "plain", evidence: "The title is a direct label." }, typographyRhythm: { status: "pass", evidence: "The rendered title matches the display role." }, contentEncoding: { status: "pass", evidence: "The opening hierarchy carries the decision." }, distanceLegibility: { status: "pass", evidence: "The title remains readable at presentation-thumbnail scale." }, grouping: { status: "not-applicable", evidence: "No peer group is declared.", rationale: "The page is a single statement." }, readingPath: { status: "not-applicable", evidence: "No relationship diagram is declared.", rationale: "The page is a single statement." } } }], checks: Object.fromEntries(["audienceBoundary", "audienceCoverage", "headlineVoice", "typographyRhythm", "contentEncoding", "layoutRhythm", "relationshipSemantics", "groupingSemantics"].map((key) => [key, key === "relationshipSemantics" || key === "groupingSemantics" ? { status: "not-applicable", anchors: ["deck"], evidence: `No ${key} structure is present.`, rationale: `No ${key} structure is declared.` } : { status: "pass", anchors: ["slide:opening"], evidence: `${key} was checked against the current rendered page.` }])), reviewerRetell: { observedBeforeContract: "The deck asks leadership to approve one coordinated operating model.", intendedTarget: communicationCore.retellTarget, alignment: "pass", limitation: "Independent reviewer proxy; not a human recall study." }, communicationReview: Object.fromEntries(["coreFidelity", "signatureCue", "semanticCausality", "retellAlignment", "invariantContinuity"].map((key) => [key, { status: "pass", anchor: "slide:opening", evidence: `${key} is visible in the reviewed page.`, recovery: `Revise ${key} and repeat review.` }])), findings: [] });
  hydrate(model);
