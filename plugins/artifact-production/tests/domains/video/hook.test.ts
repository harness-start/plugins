...
import * as hookEntry from "../../../src/domains/video/entries/hooks/video-production.js";
...
test("pre hook rejects a node eval command containing an approved wrapper substring", async () => {
  ...
  try {
    const command = `node -e "0" ${fileURLToPath(new URL("../../../dist/cli/harness.mjs", import.meta.url))} video release artifacts/video/launch/dist/launch.mp4`;
    const result = await runHook("pre", { cwd: root, tool_name: "Bash", tool_input: { command } });
