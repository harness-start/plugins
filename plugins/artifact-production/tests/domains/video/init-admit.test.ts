...
function run(script: string, args: string[], { env, sessionId }: { env: NodeJS.ProcessEnv; sessionId: string }) {
  ...
  return new Promise<{ code: number | null; stdout: string; stderr: string }>((resolve, reject) => {
    const child = spawn(process.execPath, [HARNESS, "video", action, ...args], { env: { ...process.env, ...env, AI_EXPERTS_SESSION_ID: sessionId, AI_EXPERTS_TRIGGER_FROM: "test" }, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
async function authorize(script: string, args: string[], cwd: string, sessionId: string) {
  const action = script.replace(/^project-|\.mjs$/gu, "");
  const command = ["node", HARNESS, "video", action, ...args].map((value) => JSON.stringify(value)).join(" ");
