...
export function validPosterModel(stage: PosterStage = "source"): PosterModel {
  ...
  const png = makePng();
  const svg = makeSvg();
  const reviewChecks = Object.fromEntries([
    "hierarchy", "typography", "scriptTypography", "composition", "negativeSpace", "focalDominance", "legibility", "clipping", "color", "colorSystem", "materialLighting", "copy", "profileFidelity", "assetIntegrity",
  ].map((key) => [key, { status: "pass", anchor: "variant:main", evidence: `${key} was inspected at full and thumbnail size.`, recovery: `Revise ${key} and rerun independent review.` }]));
  const communicationCore = { coreIntent: "Make the public event immediately recognizable as the next action.", audienceOutcome: "Digital readers notice the event title and know what to attend.", retellTarget: "Launch is the public event to attend.", signatureCue: { description: "The decisive event title in the lower field", semanticRole: "Primary event invitation", anchors: ["layer:background-base"] }, semanticLink: "The focal title carries the event invitation instead of acting as decoration.", invariants: ["title remains the dominant focal layer"], prohibitedDrift: ["decorative metadata competes with the event title"] };
  const files: FileMap = {
    ...
    "plan.art-direction.json": JSON.stringify({ schema: ART_DIRECTION_SCHEMA, profile: "editorial", communicationCore, brief: { audience: "digital readers", objective: "announce a public event", environment: "mobile feed and editorial listing" }, constraints: { mustKeep: ["exact event title"], mayChange: ["crop and scale"], avoid: ["decorative metadata"] }, concept: "A type-led editorial announcement.", visualCenter: "One decisive event title.", hierarchy: "Title, date, supporting copy.", typographyStrategy: "Role-based scale and spacing.", colorRationale: "High-contrast restrained palette.", letterform: { typeClass: "geometric sans", strokeProfile: "uniform", structure: "stable horizontal gravity", edgeFinish: "clean vector", sceneReference: "public event editorial title" }, composition: { dominantAxis: "asymmetric-grid", focalRelationship: "title anchors the lower field", massToVoidTarget: { min: 0, max: 0.4 }, primaryFocalLayer: "background-base", focalBox: { x: 0, y: 0, width: 1, height: 1 }, quietRegions: [{ id: "upper-air", box: { x: 0, y: 0, width: 1, height: 0.25 }, maxOccupancy: 0.05 }], titleMediaRelation: { depth: "separate", mechanism: "none" } }, material: { primary: "uncoated paper", surfaceResponse: "soft diffuse grain" }, lighting: { direction: "top-left", quality: "soft", contrast: "restrained" }, negativeRules: ["pseudo-text", "decorative-metadata"] }),
    ...
    "design.system.json": JSON.stringify({ schema: DESIGN_SYSTEM_SCHEMA, colors: { tokens: { canvas: "F4F0E8", textPrimary: "111111", accent: "7A4E32" }, core: "accent", structuralRoles: { canvas: "canvas", primaryText: "textPrimary", accent: "accent" }, scenarios: [{ id: "default", roles: { canvas: "canvas", primaryText: "textPrimary", accent: "accent" } }] }, typography: { display: { families: { cjk: "Noto Sans SC", latin: "Noto Sans SC" }, hierarchy: 1, orientation: "horizontal", alignment: "left", trackingPolicy: "neutral CJK with measured Latin tracking", sizePx: 72, weight: 700, lineHeightPx: 80, letterSpacingEm: 0, maxWidthPx: 280, maxLines: 2, scriptPolicy: "mixed" } }, fontRegistry: [{ family: "Noto Sans SC", package: "@fontsource/noto-sans-sc", files: [{ path: "noto-sans-sc-latin-700-normal.woff", weight: 700, script: "latin" }, { path: "noto-sans-sc-chinese-simplified-700-normal.woff", weight: 700, script: "cjk" }] }], spacing: { safeAreaPx: 24, baseUnitPx: 8, paragraphGapPx: 16 }, contrastPairs: [{ foreground: "textPrimary", background: "canvas", minimum: 4.5 }] }),
    ...
    "src/variants/manifest.json": JSON.stringify({ schema: VARIANT_MANIFEST_SCHEMA, variants: [{ index: 1, id: "main", directory: "001-main", width: 320, height: 320, colorScenario: "default" }] }),
    "src/variants/001-main/variant.json": JSON.stringify({ schema: "poster-production/variant/v3", id: "main", width: 320, height: 320, data: "data/main.json", colorScenario: "default" }),
    "src/variants/001-main/layers/manifest.json": JSON.stringify({ schema: LAYER_MANIFEST_SCHEMA, layers: [{ index: 1, id: "background-base", role: "background", visualRole: "background", source: "001-background-base.tsx" }] }),
  ...
  if (["probe", "review", "release"].includes(stage)) {
    ...
    files["evidence.composition.json"] = JSON.stringify({ schema: COMPOSITION_EVIDENCE_SCHEMA, plugin: "poster-production", artifactId, subjectDigest, verdict: "pass", measurements: [{ id: "main", foregroundCoverage: 0, voidCoverage: 1, focal: { layerId: "background-base", bbox: { x: 0, y: 0, width: 1, height: 1 }, centroid: { x: 0.5, y: 0.5 }, withinDeclaredBox: true }, quietRegions: [{ id: "upper-air", occupancy: 0, maximum: 0.05, status: "pass" }], titleMediaRelation: { depth: "separate", mechanism: "none", overlapRatio: 0, orderMatches: true }, status: "pass" }] });
  }
  if (["review", "release"].includes(stage)) {
    files["review.poster.json"] = JSON.stringify({ schema: REVIEW_SCHEMA, plugin: "poster-production", artifactId, subjectDigest: computePosterSubjectDigest(model), verdict: "pass", reviewer: { kind: "independent-agent", id: "reviewer-1", sessionId: "review-session" }, variants: [{ id: "main", pngSha256: sha256(png), verdict: "pass" }], checks: reviewChecks, reviewerRetell: { observedBeforeContract: "A public event named Launch is the dominant message.", intendedTarget: communicationCore.retellTarget, alignment: "pass", limitation: "Independent reviewer proxy; not a human recall study." }, communicationReview: Object.fromEntries(["coreFidelity", "signatureCue", "semanticCausality", "retellAlignment", "invariantContinuity"].map((key) => [key, { status: "pass", anchor: "layer:background-base", evidence: `${key} is visible in the reviewed poster.`, recovery: `Revise ${key} and repeat review.` }])), findings: [] });
  }
  if (stage === "release") {
