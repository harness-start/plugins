...
import * as hookEntry from "../../../src/domains/poster/entries/hooks/poster-production.js";
import { evaluatePosterShell } from "../../../src/domains/poster/lib/shell-policy.js";
...
test("Claude and Codex owner routes both register PostToolUseFailure", () => {
  ...
  assert.ok(readModuleRoutes(import.meta.url, "codex", "poster").PostToolUseFailure);
});
...
function runMode(mode, event) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "codex", ({ "session-start": "SessionStart", pre: "PreToolUse", post: "PostToolUse", failure: "PostToolUseFailure", stop: "Stop", session: "SessionStart", prompt: "UserPromptSubmit", "user-prompt": "UserPromptSubmit", subagent: "SubagentStart", "subagent-stop": "SubagentStop" } as Record<string, string>)[mode] ?? mode], { stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    child.stdout.on("data", (chunk) => { stdout += chunk; });
    child.stderr.on("data", (chunk) => { stderr += chunk; });
...
test(`pre hook allows an exact registered ${writer} invocation`, async () => {
  ...
  try {
    ...
    const wrapper = fileURLToPath(new URL("../../../dist/cli/harness.mjs", import.meta.url));
    const action = writer.replace(/^project-|\.mjs$/gu, "");
    ...
    const result = await run({
      ...
      tool_input: { command: `node ${wrapper} poster ${action} artifacts/poster/launch${suffix}` },
    });
    assert.equal(result.code, 0);
