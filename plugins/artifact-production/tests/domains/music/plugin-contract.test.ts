...
import { EXTERNAL_SKILLS } from "../../../src/domains/music/lib/contract.js";

const repositoryRoot = fileURLToPath(new URL("../../../../../", import.meta.url));
const ownerRoot = join(repositoryRoot, "plugins", "artifact-production");
const pluginRoot = ownerRoot;
...
test("publishes the renamed music guard with separate authoring and review skills", async () => {
  const claude = await json(join(ownerRoot, ".claude-plugin", "plugin.json"));
  const codex = await json(join(ownerRoot, ".codex-plugin", "plugin.json"));
  assert.equal(claude.name, "artifact-production");
  assert.equal(codex.name, "artifact-production");
  assert.equal(claude.version, "1.0.0");
  assert.equal(codex.version, "1.0.0");

  const authoring = await readFile(join(ownerRoot, "skills", "music-project-authoring", "SKILL.md"), "utf8");
  const review = await readFile(join(ownerRoot, "skills", "music-project-review", "SKILL.md"), "utf8");
...
test("uses a bundled bilingual first-party adviser pool and exposes controlled seams", async () => {
  ...
  for (const name of EXTERNAL_SKILLS.map((entry) => entry.name)) {
    const skill = await readFile(join(ownerRoot, "skills", name, "SKILL.md"), "utf8");
  ...
  for (const entry of ["project-advice", "project-reference", "project-review", "project-stage"]) {
    await readFile(join(pluginRoot, "src", "domains", "music", "entries", "cli", `${entry}.ts`), "utf8");
  }
  await readFile(join(pluginRoot, "src", "domains", "music", "lib", "capability.ts"), "utf8");
  await readFile(join(pluginRoot, "src", "domains", "music", "lib", "shell-policy.ts"), "utf8");
});

test("bundled composition-method integrity check passes in a clean consumer tree", async () => {
  const script = join(ownerRoot, "skills", "music-composition-method", "scripts", "music_theory_sanity_check.py");
