...
async function run(root) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "music", "optimize", root], { stdio: ["ignore", "pipe", "pipe"] });
    let stderr = "";
...
test("writes digest-named symbolic score and metrics through the optimizer CLI", async () => {
  ...
  try {
    ...
    await issueMusicWriterCapability({ root, capability: "music-optimize", argv: [ENTRY, "music", "optimize", root], subjectDigest: computeMusicSubjectDigest(model), sessionId: "test-optimize", triggerFrom: "test" });
