...
import { computeMusicSubjectDigest, musicReviewArtifactPaths } from "../../../src/domains/music/lib/contract.js";
import { collectMusicModel, releaseProject, validateListeningReview } from "../../../src/domains/music/lib/release.js";
import { encodePcm16Wav } from "../../../src/domains/music/lib/wav.js";
...
test("releases only a reviewed current mix and writes digest-bound evidence", async () => {
  ...
  const files = {
    ...
    "package.json": "{}\n",
    "package-lock.json": "{}\n",
    "plan.contract.json": JSON.stringify({ schema: "music-production/plan/v1", artifactId: "study", targetStage: "release" }),
    "plan.brief.json": JSON.stringify({ schema: "music-production/brief/v1", artifactId: "study", language: "en", audience: "listeners", useCase: "cue", durationSeconds: 8, mood: "bright", genre: "electronic", referenceTraits: ["hook"], structure: ["main"], instrumentation: ["lead"], constraints: ["synth only"], prohibitedDirections: ["copying"], successCriteria: ["clear motif"] }),
  ...
  await writeFile(join(root, previewPath), JSON.stringify({ schema: "music-production/preview/v1", subjectDigest: sourceDigest, mixSha256: sha256(wav), stems: { [stemPath]: sha256(wav) } }));
  ...
  const review = { schema: "music-production/review/v1", plugin: "music-production", artifactId: "study", subjectDigest: sourceDigest, mixSha256: sha256(wav), decision: "approved", reviewer: { kind: "independent-agent", id: "reviewer", sessionId: "review-session" }, coverage, checks, findings: [] };
...
test("accepts only a structured approved review bound to the current mix", () => {
  ...
  assert.equal(validateListeningReview({ schema: "music-production/review/v1", decision: "changes_requested", subjectDigest: binding.sourceDigest, mixSha256: binding.mixSha256 }, binding), false);
  assert.equal(validateListeningReview({ schema: "music-production/review/v1", decision: "approved", subjectDigest: binding.sourceDigest, mixSha256: binding.mixSha256 }, binding), true);
});
