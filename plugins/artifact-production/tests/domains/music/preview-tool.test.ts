...
test("preview consumes the current render without rewriting the mix", async () => {
  ...
  try {
    await writeFile(join(root, "music.project.json"), JSON.stringify({ schema: "music-production/project/v1", artifactId: "study", sampleRate: 48000, channels: 2, tracks: [{ index: 1, id: "lead", role: "melody", instrument: "src/instruments/lead.mjs" }] }));
    ...
    const argv = [ENTRY, "music", "preview", root, "--evidence-only"];
