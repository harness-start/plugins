...
import { consumeMusicWriterCapability, issueMusicWriterCapability } from "../../../src/domains/music/lib/capability.js";
...
test("writer capabilities are session, argv, subject, expiry, and one-shot bound", async () => {
  ...
  const argv = ["/plugin/dist/cli/harness.mjs", "music", "render", root];
  try {
...
test("an invalid bootstrap grant does not create its prospective project root", async () => {
  ...
  try {
    await assert.rejects(() => issueMusicWriterCapability({
      ...
      argv: ["/plugin/dist/cli/harness.mjs", "music", "init", root],
