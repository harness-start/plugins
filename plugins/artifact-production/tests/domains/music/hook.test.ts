...
import { consumeMusicWriterCapability } from "../../../src/domains/music/lib/capability.js";
import * as hookEntry from "../../../src/domains/music/entries/hooks/music-production.js";
import { evaluateMusicShell } from "../../../src/domains/music/lib/shell-policy.js";
...
const REFERENCE_ENTRY = fileURLToPath(new URL("../../../dist/cli/harness.mjs", import.meta.url));
const INIT_ENTRY = fileURLToPath(new URL("../../../dist/cli/harness.mjs", import.meta.url));
...
async function runPre(command, cwd = process.cwd(), env = process.env) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "codex", "PreToolUse"], { stdio: ["pipe", "pipe", "pipe"], env });
    let stdout = "";
...
test("pre hook denies a direct symbolic score write", async () => {
  const result = await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "codex", "PreToolUse"], { stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    child.stdout.on("data", (chunk) => { stdout += chunk; });
    child.on("error", reject);
    child.on("close", (code) => resolve({ code, stdout }));
...
test("denies a shell payload that merely smuggles a registered wrapper path as an argument", async () => {
  const result = await runPre("node -e \"require('fs').writeFileSync('artifacts/music/study/dist/forged.wav','x')\" plugins/artifact-production/dist/cli/harness.mjs music render artifacts/music/study");
  assert.equal(result.code, 0);
  assert.equal(JSON.parse(result.stdout).hookSpecificOutput.permissionDecision, "deny");
});
...
test("issues a one-shot capability only for an exact registered writer command", async () => {
  ...
  const command = `node "${RENDER_ENTRY}" music render "${root}"`;
  ...
  assert.equal(result.code, 0);
  assert.equal(result.stdout, "");
  const grant = await consumeMusicWriterCapability({ root, capability: "music-render", argv: [RENDER_ENTRY, "music", "render", root] });
...
test("issues a bootstrap capability for the exact project initializer", async () => {
  ...
  const command = `node "${INIT_ENTRY}" music init "${root}" --skip-install`;
  ...
  assert.equal(result.code, 0);
  assert.equal(result.stdout, "");
  const grant = await consumeMusicWriterCapability({ root, capability: "music-init", argv: [INIT_ENTRY, "music", "init", root, "--skip-install"] });
...
test("issues a reference capability only for the exact two-input writer command", async () => {
  ...
  const command = `node "${REFERENCE_ENTRY}" music reference "${root}" "${sources}" "${profile}"`;
  ...
  assert.equal(result.code, 0);
  assert.equal(result.stdout, "");
  const grant = await consumeMusicWriterCapability({ root, capability: "music-reference", argv: [REFERENCE_ENTRY, "music", "reference", root, sources, profile] });
...
test("allows a recursive Stop after the first contract block to prevent a host loop", async () => {
  ...
  const result = await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [ENTRY, "codex", "Stop"], { stdio: ["pipe", "pipe", "pipe"] });
    let stderr = "";
