...
import { pdfPageCount, toolVersion } from "../../lib/office.js";
import {
...
async function main() {
  ...
  const typographyChecks = Object.entries(typographyRoles).map(
    ([role, raw]) => {
      ...
      const pass =
        Number(value.fontSizePt) > 0 &&
        Number(value.lineSpacingMultiple) >= 1 &&
        Number(value.lineSpacingMultiple) <= 2 &&
        Number.isFinite(value.charSpacingPt) &&
        Number(value.charSpacingPt) >= -1 &&
        Number(value.charSpacingPt) <= 10 &&
        Number.isInteger(value.maxLines) &&
        Number(value.maxLines) > 0 &&
        ["cjk", "latin", "mixed"].includes(String(value.scriptPolicy));
  ...
  for (const slide of slides) {
    ...
    const pass =
      ...
      accessibility.title.length > 0 &&
      Array.isArray(accessibility.readingOrder) &&
      accessibility.readingOrder.length > 0 &&
      Array.isArray(accessibility.colorEncoding) &&
      accessibility.colorEncoding.some((channel) => channel !== "color");
    if (!pass)
