...
const PROFILE_DEFAULTS: Record<VideoProfile, { width: number; height: number; fps: number; durationInFrames: number; platform: string }> = {
...
function generateLockfile(root: string) {
...
async function main() {
  ...
  const profile = process.argv[4] as VideoProfile;
  ...
  const direction = json({ schema: DIRECTION_SCHEMA, communicationCore: { coreIntent: "Make one visible transformation easy to recover.", audienceOutcome: "The audience can repeat what changed and why it matters.", retellTarget: "One visible transformation leads to a resolved outcome.", signatureCue: { description: "The opening state transformation", semanticRole: "Primary narrative change", anchors: ["beat:opening"] }, semanticLink: "The state change directly demonstrates the promised outcome instead of decorating it.", invariants: ["the transformed object remains traceable across the timeline"], prohibitedDrift: ["decorative motion without a meaningful state change"] }, motionThesis: `Show ${artifactId} changing from an initial state to a resolved outcome.`, visualMetaphor: "one continuous visual system", narrativeArc: "hook, mechanism, proof, resolution", motionGrammar: ["transform", "route", "resolve"], negativeRules: ["slide-deck pacing", "decorative motion", "unlicensed media", "credential files"] });
  const script = json({ schema: SCRIPT_SCHEMA, beats: [], claims: [] });
  const storyboard = json({ schema: STORYBOARD_SCHEMA, beats: [{ index: 1, id: "opening", startFrame: 0, endFrame: defaults.durationInFrames, narrativeJob: "introduce and resolve the primary transformation", coreContribution: "States and demonstrates the retell target.", movingObject: "primary subject", stateChange: "changes from the initial state to the resolved outcome", cameraMotion: "stable", textRole: "headline", assetIds: [], pptRisk: "static title card" }] });
