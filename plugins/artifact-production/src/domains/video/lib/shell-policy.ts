...
function wrapperInvocation(words: string[] | null, cwd: string, workspaceRoot: string) {
  ...
  if (first === undefined || second === undefined || resource !== "video" || action === undefined || rootWord === undefined) return null;
  ...
  if (dirname(resolve(script)) !== resolve(TOOL_DIRECTORY) || basename(script) !== "harness.mjs" || !WRITERS.has(name)) return null;
  ...
  if (name === "project-init.mjs" && (words.length !== 9 || words[5] !== "--profile" || !PROFILES.has(words[6] ?? "") || words[7] !== "--mode" || !["guided", "autonomous"].includes(words[8] ?? ""))) return null;
