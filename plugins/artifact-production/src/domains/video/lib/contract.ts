...
export const PROBE_SCHEMA = "video-production/probe-evidence/v1";
export const AUDIO_EVIDENCE_SCHEMA = "video-production/audio-evidence/v1";
export const MOTION_EVIDENCE_SCHEMA = "video-production/motion-evidence/v1";
export const CAPTION_EVIDENCE_SCHEMA = "video-production/caption-evidence/v1";
export const REFERENCE_EVIDENCE_SCHEMA = "video-production/reference-evidence/v1";
...
export const REFERENCES_SCHEMA = "video-production/references/v1";
export const DESIGN_SYSTEM_SCHEMA = "video-production/design-system/v1";
export const PROJECT_SCHEMA = "video-production/project/v2";
export const BLACK_FRAME_THRESHOLD = Object.freeze({ yAvgMax: 20, yMaxMax: 32 });
...
const CAPABILITY_PATH = /^\.tmp\/video-guard\/capability\.video-(?:init|admit|render|probe|review|release|shot-stage)\.json$/u;
...
function planningDigest(model: VideoModel, stage: "direction" | "storyboard" | "assets") {
  const paths = stage === "direction"
    ? ["plan.direction.json"]
    : stage === "storyboard"
      ? ["plan.script.json", "plan.storyboard.json", ...(hasFile(model, "plan.shots.json") ? ["plan.shots.json"] : [])]
      : ["plan.assets.json"];
  if (paths.length === 1) return fileDigest(model, paths[0] ?? "");
  return sha256(paths.map((filePath) => `${filePath}\0${fileDigest(model, filePath)}\n`).join(""));
}

function validateShotPlan(model: VideoModel, stage: string | undefined, findings: VideoFinding[]) {
  ...
  const storyboard = (() => {
    try { return JSON.parse(model.files?.["plan.storyboard.json"] ?? "") as unknown; } catch { return null; }
  })();
  ...
  for (const selection of shotPlan.selections) {
    ...
    } catch { catalogValid = false; }
    const valid = beat !== undefined
      && !covered.has(beatId)
      && ["direct", "adapted", "inspired"].includes(String(selection.usage))
      && typeof selection.adaptationNotes === "string"
      && selection.adaptationNotes.trim().length > 0
      && implementationPath === path.normalize(implementationPath)
      && implementationPath.startsWith("src/visual/")
      && hasFile(model, implementationPath)
      && reviewFrames.length >= 2
      && reviewFrames.every((frame) => Number.isInteger(frame) && frame >= Number(beat.startFrame) && frame < Number(beat.endFrame))
      && new Set(reviewFrames).size === reviewFrames.length
      && catalogValid;
    ...
    if (beat !== undefined && !covered.has(beatId)) covered.add(beatId);
  }
...
function validateToolchain(files: VideoFileMap, findings: VideoFinding[]) {
  ...
  const plan = (() => { try { return JSON.parse(files["plan.contract.json"] ?? "") as unknown; } catch { return null; } })();
...
function validateProbeEvidence(model: VideoModel, findings: VideoFinding[]) {
  ...
  const validMotionSample = (sample: unknown) => {
  ...
  const expectedCandidates = new Map<number, Record<string, unknown>>();
  ...
  const candidateFrames = blackCandidates.map((candidate) => isObject(candidate) ? candidate.frame : undefined);
  const candidatesValid = blackCandidates.length === expectedCandidates.size && new Set(candidateFrames).size === candidateFrames.length && blackCandidates.every((candidate) => {
    ...
    return expected !== undefined && candidate.sha256 === expected.sha256 && JSON.stringify(candidate.luma) === JSON.stringify(expected.luma);
  });
  ...
  if (declaredShots.length > 0) {
    ...
    const validSelections = declaredShots.every((selection) => {
      ...
      return isObject(evidence)
        && evidence.recipeId === selection.recipeId
        && evidence.styleId === selection.styleId
        && evidence.usage === selection.usage
        && evidence.implementationPath === selection.implementationPath
        && evidence.implementationSha256 === fileDigest(model, String(selection.implementationPath ?? ""))
        && reviewFrames.length === expectedFrames.length
        && expectedFrames.every((frame) => reviewFrames.some((sample) => isObject(sample) && sample.frame === frame && sixDigitHash(sample.sha256)));
    });
...
function validateReviewEvidence(model: VideoModel, findings: VideoFinding[]) {
  ...
  const motion = evidenceObject(model, "evidence.motion.json");
  const blackCandidateFrames = Array.isArray(motion?.blackCandidates) ? motion.blackCandidates.map((candidate) => isObject(candidate) ? candidate.frame : undefined).filter((frame): frame is number => Number.isInteger(frame)) : [];
  ...
  }) || new Set(frameIndexes).size !== frameIndexes.length || !frameIndexes.includes(0) || !frameIndexes.includes((duration as number) - 1) || !requiredShotFrames.every((frame) => frameIndexes.includes(frame)) || !blackCandidateFrames.every((frame) => frameIndexes.includes(frame))) findings.push(finding("FRAME_EVIDENCE_INVALID", "evidence.frames.json", "frame evidence must bind unique start, interior, final, selected shot-review, and near-black candidate frame hashes"));
  ...
  const expectedShotDigest = hasFile(model, "evidence.shots.json") ? fileDigest(model, "evidence.shots.json") : undefined;
  const blackFrameAssessments = Array.isArray(review?.blackFrameAssessments) ? review.blackFrameAssessments : [];
  const assessedFrames = blackFrameAssessments.map((assessment) => isObject(assessment) ? assessment.frame : undefined);
  const blackFramesReviewed = blackFrameAssessments.length === blackCandidateFrames.length
