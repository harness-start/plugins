...
import { assertDiagramProjectRoot } from "./writer.js";
...
const pathOf = (root: string, capability: string) => join(root, ".tmp", "diagram-guard", `capability.${capability}.json`);
...
export async function issueWriterCapability({ root: rawRoot, capability, argv, subjectDigest, sessionId, triggerFrom }: { root: string; capability: string; argv: unknown; subjectDigest: string; sessionId: string; triggerFrom?: string }) {
  ...
  const directory = join(root, ".tmp", "diagram-guard"); const target = pathOf(root, capability); await mkdir(directory, { recursive: true });
  try { const existing = JSON.parse(await readFile(target, "utf8")) as Record<string, unknown>; if (Number(existing.expiresAt) >= Date.now()) throw new Error("WRITER_CAPABILITY_BUSY"); await unlink(target); }
  catch (error) { if (error instanceof Error && error.message === "WRITER_CAPABILITY_BUSY") throw error; if (errorCode(error) !== "ENOENT" && !(error instanceof SyntaxError)) throw error; if (error instanceof SyntaxError) await unlink(target).catch(() => undefined); }
  const grant: WriterCapabilityGrant = { schema: "diagram-production/writer-capability/v1", id: randomUUID(), capability, root, argvSha256: argvDigest(argv), subjectDigest, sessionId, triggerFrom: triggerFrom || "PreToolUse", issuedAt: new Date().toISOString(), expiresAt: Date.now() + TTL_MS };
  await writeFile(target, `${JSON.stringify(grant)}\n`, { flag: "wx", mode: 0o600 }); await chmod(target, 0o600); return grant;
}

export async function consumeWriterCapability({ root: rawRoot, capability, argv }: { root: string; capability: string; argv: unknown }) {
  const root = assertDiagramProjectRoot(rawRoot, { allowMissing: capability === "diagram-init" }); const target = pathOf(root, capability); let bytes: Buffer;
  try { const metadata = await lstat(target); if (!metadata.isFile() || metadata.isSymbolicLink()) throw new Error("WRITER_CAPABILITY_INVALID"); bytes = await readFile(target); await unlink(target); }
  catch (error) { if (errorCode(error) === "ENOENT") throw new Error("WRITER_CAPABILITY_MISSING"); throw error; }
  let grant: unknown; try { grant = JSON.parse(bytes.toString("utf8")) as unknown; } catch { throw new Error("WRITER_CAPABILITY_INVALID"); }
  if (typeof grant !== "object" || grant === null) throw new Error("WRITER_CAPABILITY_INVALID"); const value = grant as Record<string, unknown>;
...
export function processWriterArgv() { return currentOwnerCliArgv(); }
