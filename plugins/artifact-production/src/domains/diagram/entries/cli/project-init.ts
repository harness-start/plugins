...
import { DESIGN_SYSTEM_SCHEMA, PLAN_SCHEMA, PROJECT_SCHEMA, SOURCE_SCHEMA } from "../../lib/contract.js";
import { consumeWriterCapability, processWriterArgv } from "../../lib/capability.js";
import { assertDiagramProjectRoot, atomicWrite, withWriterJournal } from "../../lib/writer.js";

function runNpm(root: string) {
  return new Promise<void>((resolve, reject) => {
    const child = spawn("npm", ["install", "--ignore-scripts", "--no-audit", "--no-fund"], { cwd: root, stdio: ["ignore", "ignore", "pipe"] }); let stderr = "";
    const timer = setTimeout(() => child.kill("SIGKILL"), 180_000); child.stderr.on("data", (chunk) => { if (stderr.length < 1024 * 1024) stderr += chunk; });
    child.on("error", (error) => { clearTimeout(timer); reject(new Error(`NPM_UNAVAILABLE:${error.message}`)); }); child.on("close", (code) => { clearTimeout(timer); if (code === 0) resolve(); else reject(new Error(`NPM_INSTALL_FAILED:${stderr.trim()}`)); });
  });
}

async function main() {
  const root = assertDiagramProjectRoot(process.argv[2], { allowMissing: true }); const grant = await consumeWriterCapability({ root, capability: "diagram-init", argv: processWriterArgv() }); const artifactId = basename(root);
...
await main().catch((error: unknown) => { process.stderr.write(`[diagram-project-init] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
