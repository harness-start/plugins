...
function wrapperInvocation(words: string[] | null, cwd: string, workspaceRoot: string) {
  ...
  if (!runtime || !entry || resource !== "poster" || !action || !rootWord || !["node", basename(process.execPath), process.execPath].includes(runtime) || entry.startsWith("-")) return null;
...
export function evaluatePosterShell({ command, cwd, workspaceRoot }: { command: unknown; cwd: string; workspaceRoot: string; activeProjectCount?: number }): PosterShellDecision {
