...
function mergedMask(masks: Uint8Array[], length: number): Uint8Array {
  const output = new Uint8Array(length);
  for (const mask of masks) for (let index = 0; index < length; index += 1) if (mask[index]) output[index] = 1;
  return output;
}

function maskOverlap(left: Uint8Array, right: Uint8Array): number {
  let leftCount = 0;
  let rightCount = 0;
  let intersection = 0;
...
await main().catch((error: unknown) => { process.stderr.write(`[poster-project-probe] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
