...
export async function withWriterJournal<T>(root: string, capability: string, callback: () => Promise<T>, grant: WriterSessionGrant = {}) {
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schemaVersion: 1, plugin: "video-production", operation: capability, artifactId: basename(root), ...sessionMetadata(capability, grant) })}\n`);
    await handle.sync();
  } finally {
    await handle.close();
  }
