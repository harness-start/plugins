...
import { evaluateTrainingShell, parseShellWords } from "../src/lib/shell-policy.js";
...
test("shell policy admits an exact registered training writer", () => {
  ...
  assert.deepEqual(
    evaluateTrainingShell({ command: `node "${script}" "${projectRoot}"`, cwd: workspaceRoot, workspaceRoot }),
    { decision: "allow", writer: "training-render", projectRoot, argv: [script, projectRoot] },
  );
});

test("shell policy rejects command chains and unknown mutation tools in training scope", () => {
  const script = resolve(PLUGIN_ROOT, "dist/cli/project-render.mjs");
  for (const command of [`node "${script}" "${projectRoot}" && touch forged`, `python /tmp/course-writer.py ${projectRoot}`, `rm -rf ${projectRoot}`]) {
