...
import {
  ...
  validateMusicModel,
  validateMusicReview
} from "./chunk-DBL3PMOB.mjs";

// plugins/artifact-production/modules/music/src/lib/wav.ts
...
// plugins/artifact-production/modules/music/src/lib/release.ts
...
function validateListeningReview(review, { sourceDigest, mixSha256 }) {
  ...
  const record = review;
  return record.schema === "music-production/review/v1" && record.subjectDigest === sourceDigest && record.mixSha256 === mixSha256 && record.decision === "approved";
}
async function releaseProject(inputRoot) {
  ...
  await journal.writeFile(`${JSON.stringify({ schemaVersion: 1, plugin: "music-production", operation: "release", sourceDigest, sessionId: process.env.AI_EXPERTS_SESSION_ID ?? "unknown" })}
`);
  await journal.sync();
  await journal.close();
  let complete = false;
  try {
    ...
    await writeJsonAtomic(join(root, "evidence.audio.json"), {
      schema: "music-production/audio-evidence/v1",
    ...
    await writeJsonAtomic(join(root, "release.manifest.json"), {
      schema: "music-production/release/v2",
      engine: MUSIC_ENGINE,
      artifactId: initial.artifactId,
      sourceDigest,
      outputs: {
