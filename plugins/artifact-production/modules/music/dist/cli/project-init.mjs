...
// plugins/artifact-production/modules/music/src/entries/cli/project-init.ts
import { spawn } from "node:child_process";
import { createHash } from "node:crypto";
import { mkdir, readdir, writeFile } from "node:fs/promises";
import { basename, join, resolve } from "node:path";
var args = process.argv.slice(2);
var root = resolve(args[0] ?? "");
var id = basename(root);
...
async function main() {
  ...
  const expectedSubject = createHash("sha256").update(`music-production@0.4.0
init\0${root}`).digest("hex");
  ...
  await Promise.all([
    ...
    writeJson(join(root, "plan.contract.json"), {
      schema: "music-production/plan/v1",
      artifactId: id,
      targetStage: "source"
    }),
    ...
    writeJson(join(root, "plan.direction.json"), {
      schema: "music-production/direction/v1",
      artifactId: id,
      tonalCenter: "C major",
      tempo: "120 BPM",
      meter: "4/4",
    ...
    writeJson(join(root, "plan.arrangement.json"), {
      schema: "music-production/arrangement/v1",
      artifactId: id,
...
`, { flag: "wx" })
  ...
  if (!skipInstall) {
    ...
    if (installBrowser) await run("npx", ["playwright", "install", "chromium"], root, 3e5);
  }
