...
import { evaluateDiagramShell, parseShellWords } from "../src/lib/shell-policy.js";

const workspaceRoot = "/tmp/diagram-shell-workspace";
const projectRoot = `${workspaceRoot}/artifacts/diagram/service-flow`;
const tool = resolve("plugins/artifact-production/modules/diagram/dist/cli/project-import.mjs");
...
test("registered import requires an absolute source path and exact argv", () => {
  assert.equal(evaluateDiagramShell({ command: `node ${tool} ${projectRoot} source.mmd`, cwd: workspaceRoot, workspaceRoot }).decision, "deny");
  assert.equal(evaluateDiagramShell({ command: `node ${tool} ${projectRoot} /tmp/source.mmd`, cwd: workspaceRoot, workspaceRoot }).decision, "allow");
  assert.equal(evaluateDiagramShell({ command: `node ${tool} ${projectRoot} /tmp/source.mmd extra`, cwd: workspaceRoot, workspaceRoot }).decision, "deny");
});
