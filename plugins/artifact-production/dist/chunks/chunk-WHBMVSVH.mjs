...
import {
  unzipSync
} from "./chunk-COPIZFLC.mjs";
import {
  require_lib
} from "./chunk-SELZDFVX.mjs";
import {
...
var PLAN_SCHEMA = "presentation-production/plan/v3";
var STORYBOARD_SCHEMA = "presentation-production/storyboard/v3";
...
var REVIEW_INPUT_SCHEMA = "presentation-production/review-input/v2";
var REVIEW_SCHEMA = "presentation-production/review/v3";
...
function validateSourceSchemas(model, files, findings) {
  ...
  if (storyboard && (!storyboardSlides.length || !storyboardSlides.every(
    (entry, index) => rec(entry)?.index === index + 1 && typeof rec(entry)?.id === "string" && typeof rec(entry)?.title === "string" && typeof rec(entry)?.role === "string" && typeof rec(entry)?.visualType === "string" && typeof rec(entry)?.assertion === "string" && Boolean(String(rec(entry)?.assertion).trim()) && typeof rec(entry)?.narrativeJob === "string" && Boolean(String(rec(entry)?.narrativeJob).trim()) && typeof rec(entry)?.transition === "string" && Boolean(String(rec(entry)?.transition).trim())
  )))
    findings.push(
      finding(
        "STORYBOARD_INVALID",
