...
async function main() {
  ...
  const compositionSignalPages = new Set((Array.isArray(designEvidence.compositionSignals) ? designEvidence.compositionSignals : []).filter((entry) => Array.isArray(record(entry).signals) && record(entry).signals.length > 0).map((entry) => Number(record(entry).page)));
