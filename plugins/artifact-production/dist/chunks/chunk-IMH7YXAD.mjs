...
// plugins/artifact-production/src/domains/video/lib/project.ts
...
// plugins/artifact-production/src/domains/video/lib/writer.ts
...
async function withWriterJournal(root, capability, callback, grant = {}) {
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schemaVersion: 1, plugin: "video-production", operation: capability, artifactId: basename2(root), ...sessionMetadata(capability, grant) })}
`);
    await handle.sync();
  } finally {
    await handle.close();
