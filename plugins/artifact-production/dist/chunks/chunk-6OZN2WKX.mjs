...
// plugins/artifact-production/src/domains/diagram/lib/contract.ts
import { createHash } from "node:crypto";
import { lstat, readdir, readFile } from "node:fs/promises";
import { basename, dirname, extname, join, posix, relative, resolve, sep } from "node:path";
var PLAN_SCHEMA = "diagram-production/plan/v2";
...
var REVIEW_INPUT_SCHEMA = "diagram-production/review-input/v2";
var REVIEW_SCHEMA = "diagram-production/review/v2";
...
function validateDiagramModel(model, options = {}) {
  ...
  if (!communicationCoreValid(plan?.communicationCore)) findings.push(finding("COMMUNICATION_CORE_INVALID", "plan.contract.json", "diagram plan must bind a complete communicationCore"));
    ...
    const allowed = /* @__PURE__ */ new Set([...nodeIds.map((id) => `node:${id}`), ...edgeIds.map((id) => `edge:${id}`), ...dataIds.map((id) => `data:${id}`)]);
...
// plugins/artifact-production/src/domains/diagram/lib/writer.ts
