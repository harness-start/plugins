...
import {
  pdfPageCount,
  toolVersion
} from "./chunk-6BW37HJ7.mjs";
import {
  consumeWriterCapability,
  processWriterArgv
} from "./chunk-KJJ677RC.mjs";
import {
  assertPptxProjectRoot,
  atomicWriteJson,
  sessionMetadata,
  withWriterJournal
} from "./chunk-5RPDYTZ4.mjs";
import {
  ...
  loadPptxProject,
  validatePptxModel
} from "./chunk-VZLJHBEA.mjs";
import "./chunk-JBK55I2P.mjs";
import "./chunk-VAJCBL5A.mjs";
import "./chunk-7LRQYS6E.mjs";
import "./chunk-5WHBFGBZ.mjs";
import "./chunk-ACZGLWAJ.mjs";

// plugins/artifact-production/src/domains/presentation/entries/cli/project-probe.ts
...
async function main() {
  ...
  const diagramDigests = storyboardSlides.map((slide) => record(slide.visual)).filter((visual) => visual.type === "diagram" && visual.mode === "svg").map((visual) => visual.sha256).filter((value) => typeof value === "string");
  ...
  const textRhythmPages = packageInspection.slides.map((slide) => ({
    ...
    governedTextObjects: slide.objects.filter(
      ({ name, text }) => Boolean(text) && /^(?:pptx:(?:title|text|list|item|node):)/u.test(name)
    ).length,
