...
// plugins/artifact-production/src/domains/logo/lib/png-decode.ts
...
// plugins/artifact-production/src/domains/logo/lib/contract.ts
...
var BRIEF_SCHEMA = "brand-logo-production/brief/v3";
...
var CONCEPT_SELECTION_SCHEMA = "brand-logo-production/concept-selection/v2";
...
var REVIEW_INPUT_SCHEMA = "brand-logo-production/review-input/v2";
var REVIEW_SCHEMA = "brand-logo-production/review/v3";
...
function validateBriefAndSkillComposition(model, findings) {
  ...
  if (!markStrategy || !["symbol", "wordmark", "lettermark", "monogram", "combination"].includes(String(markStrategy.carrier)) || !["use", "avoid", "not-applicable"].includes(String(markStrategy.initialismDecision)) || typeof markStrategy.rationale !== "string" || markStrategy.rationale.trim().length < 12) {
...
function validateConcepts(model, findings) {
  ...
  const assessments = asList(manifestRecord?.routeAssessments).map(rec).filter((entry) => entry !== void 0);
  let project = null;
  try {
  ...
  if (assessments.length !== CONCEPT_BUCKETS.length || new Set(assessedBuckets).size !== CONCEPT_BUCKETS.length || !CONCEPT_BUCKETS.every((bucket) => assessedBuckets.includes(bucket)) || explored.size < 3 || assessments.some((entry) => !["explore", "reject"].includes(String(entry.decision)) || typeof entry.rationale !== "string" || entry.rationale.trim().length < 12) || buckets.some((bucket) => !explored.has(String(bucket))) || [...explored].some((bucket) => !buckets.includes(bucket))) {
  ...
  concepts.forEach((entry, offset) => {
    ...
    if (!match || typeof item?.id !== "string" || !item.id || item.index !== offset + 1 || Number(match.groups?.index) !== item.index || !CONCEPT_BUCKETS.includes(String(item.bucket)) || typeof item.rationale !== "string" || item.rationale.trim().length < 12 || ["coreContribution", "memoryCue", "retellLine", "invariant", "risk"].some((key) => typeof item[key] !== "string" || String(item[key]).trim().length < 8)) {
