...
// plugins/artifact-production/src/domains/presentation/entries/cli/project-render.ts
...
async function main() {
  ...
  await withWriterJournal(root, "pptx-render", async () => {
    ...
    try {
      ...
      const diagramDigests = storyboard.slides.filter(({ visual }) => visual.type === "diagram" && visual.mode === "svg").map(({ visual }) => visual.sha256).filter((value) => typeof value === "string");
