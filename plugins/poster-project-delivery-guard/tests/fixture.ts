...
export function validPosterModel(stage: PosterStage = "source"): PosterModel {
  ...
  const files: FileMap = {
    ...
    "src/variants/001-main/variant.json": JSON.stringify({ schema: "poster-project-delivery-guard/variant/v2", id: "main", width: 320, height: 320, data: "data/main.json" }),
  ...
  if (["render", "probe", "review", "release"].includes(stage)) {
    ...
    files["evidence.render.json"] = JSON.stringify({ schema: RENDER_EVIDENCE_SCHEMA, plugin: "poster-project-delivery-guard", artifactId, subjectDigest: computePosterSubjectDigest(model), verdict: "pass", sessionId: "render-session", renderer: { satori: "0.29.0", resvg: "2.6.2" }, outputs });
  }
  if (["probe", "review", "release"].includes(stage)) {
    const subjectDigest = computePosterSubjectDigest(model);
    files["evidence.probe.json"] = JSON.stringify({ schema: PROBE_EVIDENCE_SCHEMA, plugin: "poster-project-delivery-guard", artifactId, subjectDigest, verdict: "pass", measurements: [{ id: "main", svg: { width: 320, height: 320, viewBox: [0, 0, 320, 320] }, png: { width: 320, height: 320, alphaCoverage: 1 }, svgSha256: sha256(svg), pngSha256: sha256(png), independentRasterSha256: sha256(png) }], checks: [{ criterion: "svg-png-byte-equivalence", status: "pass" }, { criterion: "bounded-nonblank-raster", status: "pass" }] });
    files["evidence.accessibility.json"] = JSON.stringify({ schema: ACCESSIBILITY_EVIDENCE_SCHEMA, plugin: "poster-project-delivery-guard", artifactId, subjectDigest, verdict: "pass", nonColorEncoding: true, checks: [{ criterion: "contrast:textPrimary:canvas", status: "pass", value: 17.1, minimum: 4.5 }, { role: "display", status: "pass", sizePx: 72 }] });
  }
  if (["review", "release"].includes(stage)) {
    files["review.poster.json"] = JSON.stringify({ schema: REVIEW_SCHEMA, plugin: "poster-project-delivery-guard", artifactId, subjectDigest: computePosterSubjectDigest(model), verdict: "pass", reviewer: { kind: "independent-agent", id: "reviewer-1", sessionId: "review-session" }, variants: [{ id: "main", pngSha256: sha256(png), verdict: "pass" }], checks: { hierarchy: "pass", typography: "pass", legibility: "pass", clipping: "pass", color: "pass", copy: "pass", profileFidelity: "pass", assetIntegrity: "pass" }, findings: [] });
  }
  if (stage === "release") {
