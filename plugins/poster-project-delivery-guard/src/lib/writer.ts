...
import { isPosterProjectRoot, resolveWorkspaceRoot } from "./contract.js";

export type WriterSessionGrant = { sessionId?: string; triggerFrom?: string };

export function assertPosterProjectRoot(value: string | undefined, { allowMissing = false } = {}) {
...
export async function atomicWriteJson(root: string, relativePath: string, payload: unknown) {
  ...
  const temporaryDirectory = join(root, ".tmp", "poster-guard");
...
export async function withWriterJournal<T>(root: string, capability: string, callback: () => Promise<T>, grant: WriterSessionGrant = {}) {
  ...
  try {
    await handle.writeFile(`${JSON.stringify({ schemaVersion: 2, plugin: "poster-project-delivery-guard", operation: capability, artifactId: basename(root), ...sessionMetadata(capability, grant) })}\n`);
    await handle.sync();
