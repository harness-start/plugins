...
import {
  ART_DIRECTION_SCHEMA, ASSET_MANIFEST_SCHEMA, DESIGN_SYSTEM_SCHEMA, LAYER_MANIFEST_SCHEMA,
  PLAN_SCHEMA, POSTER_PROFILES, PROJECT_SCHEMA, SKILL_COMPOSITION_SCHEMA, VARIANT_MANIFEST_SCHEMA,
  type PosterProfile,
...
const PROFILE_DEFAULTS: Record<PosterProfile, { width: number; height: number; concept: string; visualCenter: string }> = {
...
async function main() {
  ...
  const packageJson = {
    name: `poster-${artifactId}`, private: true, type: "module",
  ...
  const files: Record<string, string> = {
    ...
    "src/variants/001-main/variant.json": `${JSON.stringify({ schema: "poster-project-delivery-guard/variant/v2", id: "main", width: defaults.width, height: defaults.height, data: "data/main.json" }, null, 2)}\n`,
...
main().catch((error: unknown) => { process.stderr.write(`[poster-project-init] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
