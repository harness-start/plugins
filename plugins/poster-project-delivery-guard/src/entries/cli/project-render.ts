...
import { RENDER_EVIDENCE_SCHEMA, computePosterSubjectDigest, inspectPosterPng, inspectPosterSvg, loadPosterProject, validatePosterModel } from "../../lib/contract.js";
  ...
  await withWriterJournal(root, "poster-render", async () => {
    ...
    try {
      ...
      for (const variant of manifest.variants) {
        ...
        const layers = JSON.parse(String(model.files?.[`src/variants/${variant.directory}/layers/manifest.json`])) as { layers: Array<{ source: string }> };
      ...
      await atomicWriteJson(root, "evidence.render.json", { schema: RENDER_EVIDENCE_SCHEMA, plugin: "poster-project-delivery-guard", artifactId: model.artifactId, subjectDigest: computePosterSubjectDigest(model), verdict: "pass", renderer: { satori: "0.29.0", resvg: "2.6.2" }, outputs, ...sessionMetadata("poster-render", grant) });
    } finally { await rm(temp, { recursive: true, force: true }); }
  }, grant);
  process.stdout.write(`${JSON.stringify({ artifactId: model.artifactId, variants: manifest.variants.length })}\n`);
}

main().catch((error: unknown) => { process.stderr.write(`[poster-project-render] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
