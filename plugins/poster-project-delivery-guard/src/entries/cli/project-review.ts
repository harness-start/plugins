...
import { REVIEW_SCHEMA, computePosterSubjectDigest, loadPosterProject, validatePosterModel } from "../../lib/contract.js";
...
async function main() {
  ...
  let input: unknown;
  try { input = JSON.parse(bytes.toString("utf8")) as unknown; } catch { throw new Error("REVIEW_INPUT_JSON_INVALID"); }
  ...
  const manifest = JSON.parse(String(model.files?.["src/variants/manifest.json"])) as { variants: Array<{ id: string }> };
  const variants = Array.isArray(payload.variants) ? payload.variants.map(record) : [];
  if (payload.schema !== "poster-project-delivery-guard/review-input/v2" || payload.artifactId !== model.artifactId || payload.subjectDigest !== computePosterSubjectDigest(model) || payload.verdict !== "pass") throw new Error("REVIEW_INPUT_INVALID");
  ...
  await withWriterJournal(root, "poster-review", async () => {
    await atomicWriteJson(root, "review.poster.json", { schema: REVIEW_SCHEMA, plugin: "poster-project-delivery-guard", artifactId: model.artifactId, subjectDigest: computePosterSubjectDigest(model), verdict: "pass", reviewer, variants, findings: reviewFindings, checks, reviewInputSha256: createHash("sha256").update(bytes).digest("hex"), ...sessionMetadata("poster-review", grant) });
  }, grant);
...
main().catch((error: unknown) => { process.stderr.write(`[poster-project-review] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
