...
var GENERATED_PATH = /^(?:dist\/|evidence(?:\.[^/]+)?(?:\/|\.json$)|review\.poster\.json$|release\.manifest\.json$|receipt\.[^/]+\.json$)/u;
var OWNER_VIOLATION = /(?:\bzIndex\s*:|from\s+["'](?:satori|@resvg\/resvg-js|node:fs|node:child_process)["']|\b(?:fetch|setTimeout|setInterval)\s*\(|\b(?:Date\.now|Math\.random)\s*\(|\b(?:useState|useEffect|useLayoutEffect)\s*\(|<\s*(?:script|style|link|iframe)\b|https?:\/\/)/u;
var RECEIPT_EXCLUDED_PATH = /^(?:dist\/|evidence(?:\.|\/)|review\.poster\.json$|release\.manifest\.json$|receipt\.[^/]+\.json$|\.poster-delivery-journal\.json$)/u;
...
function pngProofValid(bytes) {
  return Buffer.isBuffer(bytes) && bytes.byteLength >= 8 && bytes.subarray(0, 8).equals(Buffer.from("89504e470d0a1a0a", "hex"));
}
...
function digestFileMap(model, predicate) {
  const entries = Object.entries(model?.files ?? {}).filter(([filePath, value]) => predicate(filePath) && typeof value === "string").sort(([left], [right]) => left.localeCompare(right));
