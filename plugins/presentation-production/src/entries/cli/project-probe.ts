...
import {
  ACCESSIBILITY_EVIDENCE_SCHEMA, DESIGN_EVIDENCE_SCHEMA, STRUCTURE_EVIDENCE_SCHEMA,
  computePptxSubjectDigest, inspectPptxPackage, loadPptxProject, validatePptxModel,
...
async function main() {
  ...
  const base = { plugin: "presentation-production", artifactId: model.artifactId, subjectDigest: computePptxSubjectDigest(model), ...sessionMetadata("pptx-probe", grant) };
...
main().catch((error: unknown) => { process.stderr.write(`[pptx-project-probe] ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 2; });
