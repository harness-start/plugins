...
test("GitLab runs the real FFmpeg contract beside the main validation job", () => {
  ...
  assert.equal(
    ...
    "tsx --test plugins/artifact-production/tests/domains/video/media.test.ts plugins/artifact-production/tests/domains/logo/squint.test.ts",
  );
});
...
test("the main job's FFmpeg skip is honored by the separately gated test files", () => {
  ...
  const result = spawnSync(
    ...
    [
      ...
      "tsx",
      "--test",
      "plugins/artifact-production/tests/domains/video/media.test.ts",
      "plugins/artifact-production/tests/domains/logo/squint.test.ts",
    ],
    {
      cwd: root,
      encoding: "utf8",
