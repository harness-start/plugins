...
test("registered writer must be node dist/cli/<writer> <projectRoot>", () => {
  const toolDirectory = "/plugins/presentation-production/dist/cli";
  const allowed = evaluateRegisteredWriter({
    command: "node /plugins/presentation-production/dist/cli/project-release.mjs /ws/artifacts/pptx/id",
    cwd: "/ws",
    workspaceRoot: "/ws",
    carrier: "pptx",
