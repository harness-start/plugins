...
function decodeAnsiCQuoteEscape(command: string, slashIndex: number): { value: string; endIndex: number } {
...
function skipWrapperOptions(tokens: string[], start: number, optionsWithValue: ReadonlySet<string>): number {
  let index = start;
...
export function commandInvocation(tokens: string[]): ShellInvocation | null {
  let index = 0;
...
export function splitShellLogicalLines(command: string): string[] {
  const lines: string[] = [];
  let current = "";
  let quote: "'" | "\"" | null = null;
  let escaped = false;
